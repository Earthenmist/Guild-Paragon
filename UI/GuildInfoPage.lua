...
function Info:RequestData()
    ...
    if RequestGuildChallengeInfo then pcall(RequestGuildChallengeInfo) end
...
function Info:Build(page)
    ...
    left:SetWidth(360)
    local challenges = challengePanel(left)
    ...
    function page:Refresh()
        ...
        challenges:Refresh()
        motd:SetText(read(C_GuildInfo and C_GuildInfo.GetMOTD, "string") or GP.L["Unavailable"])
        details:SetText(read(C_GuildInfo and C_GuildInfo.GetInfoText, "string") or GP.L["Unavailable"])
    ...
    for _, event in ipairs({"GUILD_RANKS_UPDATE", "GUILD_NEWS_UPDATE", "GUILD_CHALLENGE_UPDATED", "GUILD_MOTD", "GUILD_ROSTER_UPDATE", "PLAYER_GUILD_UPDATE", "PLAYER_REGEN_DISABLED", "PLAYER_REGEN_ENABLED", "PLAYER_ENTERING_WORLD", "PLAYER_LEAVING_WORLD"}) do page:RegisterEvent(event) end
