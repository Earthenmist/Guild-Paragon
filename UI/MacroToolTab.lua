...
local function refreshExecutor(queuedRows)
    ...
    if controls.hotKeyText then
        controls.hotKeyText:SetText(MacroTool:GetExecutionHotKeyText())
    end
    ...
    if controls.executionHint then
        ...
        elseif plan and plan.selectedCount and plan.selectedCount > 1 then
            ...
            controls.executionHint:SetText(plan and GP.L["Build Macro arms Paragon_Tool. Review the text before running it."] or GP.L["Pick or load a rule with queued results, then build the macro."])
        end
    end
end
...
local function updateRow(row, result)
    ...
    row.select:SetShown(result.source ~= "rosterRank")
...
local function refreshLabelFilter(settings)
    if not controls.labelButton then return end
    local text = GP.L["No label filter"]
    local Labels = GP:GetModule("Labels")
    local guildKey = GP:GetModule("Roster"):GetGuildKey()
    if settings.requiredLabelId then
        text = GP.L["Required label unavailable"]
        if settings.requiredLabelGuild == guildKey then
            for _, def in ipairs(Labels:GetAllLabelDefinitions(guildKey, false)) do
                if def.labelId == settings.requiredLabelId then text = def.name; break end
            end
        end
    end
    controls.labelButton.text:SetText(text .. "  v")
end

local function createLabelFilter(parent, anchor)
    controls.labelTitle = createLabel(parent, GP.L["Require Label"], anchor, 0, -14)
    local button = makeButton(parent, GP.L["No label filter"], 210)
    button:SetPoint("TOPLEFT", controls.labelTitle, "BOTTOMLEFT", 0, -6)
    controls.labelButton = button
    local panel = Theme:CreatePanel(parent, "panel", "border")
    panel:SetSize(210, 220)
    ...
    slider:SetPoint("TOPRIGHT", -4, -4)
    slider:SetPoint("BOTTOMRIGHT", -4, 4)
    slider:SetWidth(6)
    slider:SetOrientation("VERTICAL")
    slider:SetMinMaxValues(0, 0)
    slider:SetValueStep(1)
    local thumb = slider:CreateTexture(nil, "ARTWORK")
    thumb:SetColorTexture(unpack(Theme.color.textSecondary))
    thumb:SetSize(6, 24)
    ...
    scroll:SetScript("OnMouseWheel", function(_, delta)
        ...
        slider:SetValue(math.max(0, math.min(maximum, slider:GetValue() - delta * 26)))
    end)
    local content = CreateFrame("Frame", nil, scroll)
    content:SetWidth(198)
    ...
    button:SetScript("OnClick", function()
        ...
        local MacroTool = GP:GetModule("MacroTool")
        local guildKey = GP:GetModule("Roster"):GetGuildKey()
        local options = {{name=GP.L["No label filter"]}}
        for _, def in ipairs(GP:GetModule("Labels"):GetAllLabelDefinitions(guildKey, false)) do
            options[#options+1] = def
        end
        for _, row in ipairs(rows) do row:Hide() end
        for i, def in ipairs(options) do
            local row = rows[i] or makeButton(content, "", 198)
            rows[i] = row
            row:SetHeight(24)
            row:SetWidth(#options > 8 and 188 or 202)
            row:SetBackdropBorderColor(0, 0, 0, 0)
            row.selected = def.labelId == MacroTool:GetSettings().requiredLabelId
            ...
            row:SetScript("OnLeave", function(self)
                self:SetBackdropColor(unpack(self.selected and Theme.color.panelRaised or {0, 0, 0, 0}))
            end)
            ...
            row:SetScript("OnClick", function()
                MacroTool:SetSetting("requiredLabelId", def.labelId)
                MacroTool:SetSetting("requiredLabelGuild", def.labelId and guildKey or nil)
                panel:Hide()
                MacroToolTab:Refresh(true)
            end)
            row:Show()
        end
        local height = math.min(8, #options) * 26
        panel:SetHeight(height + 8)
        content:SetWidth(#options > 8 and 188 or 202)
        content:SetHeight(math.max(26, #options*26))
        scroll:SetPoint("BOTTOMRIGHT", #options > 8 and -14 or -4, 4)
        slider:SetMinMaxValues(0, math.max(0, #options*26 - height))
        slider:SetShown(#options > 8)
        slider:SetValue(0)
        scroll:SetVerticalScroll(0)
        panel:Show()
    end)
    controls.labelHint = createLabel(parent, GP.L["Label-filtered rules sync with updated clients."], button, 0, -8)
    controls.labelHint:SetWidth(210)
    controls.labelHint:SetWordWrap(true)
end

local function syncControls(settings)
    refreshLabelFilter(settings)
    ...
    setShown(controls.labelTitle, hasAction)
    setShown(controls.labelButton, hasAction)
    setShown(controls.labelHint, hasAction and settings.requiredLabelId ~= nil)
...
function MacroToolTab:Refresh(resetScroll)
    ...
    local executionRows = MacroTool:GetExecutionRows()
    if executionRows[1] and executionRows[1].source == "rosterRank" then
        summaryText:SetText(GP.L["Prepared roster rank change. Review the member and target rank, then press the macro keybind. Use Clear Macro to cancel."])
        for index, row in ipairs(executionRows) do row.executionIndex = index end
        selectAllQueuedRows(executionRows)
        refreshExecutor(executionRows)
        list:SetData(executionRows, resetScroll)
        return
    end
...
function MacroToolTab:Build(parent)
    ...
    createLabelFilter(options, controls.safeAllNotes)
    ...
    local keybindingsButton = makeButton(executor, L["Keybindings"], 110)
    keybindingsButton:SetPoint("LEFT", hotKeyLabel, "RIGHT", 10, 0)
    keybindingsButton:SetScript("OnClick", function() GP.UI.OpenMacroKeybindings() end)
    local hotKeyText = executor:CreateFontString(nil, "ARTWORK")
    hotKeyText:SetFontObject(Theme.font.body)
    hotKeyText:SetPoint("LEFT", keybindingsButton, "RIGHT", 10, 0)
    hotKeyText:SetText(MacroTool:GetExecutionHotKeyText())
    controls.hotKeyText = hotKeyText
    ...
    MacroToolTab:RegisterMessage("GuildParagon_RosterScanned", GP:MonitorWrap("listener.macros", refreshIfShown))
    MacroToolTab:RegisterMessage("GuildParagon_LabelsChanged", refreshIfShown)
