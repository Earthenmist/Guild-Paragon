...
LibStub("AceEvent-3.0"):Embed(MemberTooltip)
...
local function disarmHousingVisit()
    local button = frame and frame.housingVisitButton
...
local function armHousingVisit(neighborhoodGUID, houseGUID, plotID)
    local button = frame and frame.housingVisitButton
    if not button or not frame:IsShown() or not frame.houseIcon:IsShown()
    ...
    local left, bottom = frame.houseIcon:GetLeft(), frame.houseIcon:GetBottom()
    local sourceScale = frame.houseIcon:GetEffectiveScale()
    local targetScale = UIParent:GetEffectiveScale()
    if not left or not bottom or not sourceScale or not targetScale or targetScale == 0 then return end

    local scaleRatio = sourceScale / targetScale
    if UnregisterStateDriver then UnregisterStateDriver(button, "visibility") end
    button:ClearAllPoints()
    button:SetSize(149 * scaleRatio, 22 * scaleRatio)
    button:SetPoint("BOTTOMLEFT", UIParent, "BOTTOMLEFT", left * scaleRatio, (bottom - 4) * scaleRatio)
...
local function formatDate(ts)
    return ts and GP:DisplayDate("%d %b '%y", ts) or GP.L["Unknown"]
end
...
local function build()
    ...
    frame.factionIcon:SetSize(20, 20)
    frame.factionIcon:SetPoint("TOP", frame.lastOnline, "BOTTOM", 0, -3)
    ...
    frame.houseIcon = frame:CreateTexture(nil, "ARTWORK")
    frame.houseIcon:SetSize(14, 14)
    if frame.houseIcon.SetAtlas then
        frame.houseIcon:SetAtlas("housing-map-plot-player-house")
    else
        frame.houseIcon:SetTexture("Interface\\MINIMAP\\TRACKING\\StableMaster")
    end

    frame.houseText = createText(frame, Theme.font.small, "LEFT")
    frame.houseText:SetWidth(130)
    frame.houseText:SetTextColor(unpack(Theme.color.accent))

    -- The protected visit action is isolated under UIParent and positioned
    -- numerically over the address after its session-only house handle resolves.
    frame.housingVisitButton = CreateFrame("Button", nil, UIParent, "SecureActionButtonTemplate")
    frame.housingVisitButton:SetFrameStrata("TOOLTIP")
    frame.housingVisitButton:SetFrameLevel(310)
    frame.housingVisitButton:RegisterForClicks("AnyDown", "AnyUp")
    frame.housingVisitButton:Hide()
    frame.housingVisitButton:SetScript("OnEnter", function()
        hideSerial = hideSerial + 1
        frame.houseText:SetTextColor(unpack(Theme.color.warning))
        GameTooltip:SetOwner(frame.housingVisitButton, "ANCHOR_RIGHT")
        GameTooltip:SetText(GP.L["Subdivision"])
        GameTooltip:AddLine(GP.L["Click to visit"], unpack(Theme.color.accent))
        GameTooltip:Show()
    end)
    frame.housingVisitButton:SetScript("OnLeave", function()
        frame.houseText:SetTextColor(unpack(Theme.color.accent))
        GameTooltip_Hide()
        MemberTooltip:HideSoon(activeAnchor)
    end)
    frame.housingVisitButton:SetScript("PostClick", function()
        C_Timer.After(0, function() MemberTooltip:Hide() end)
    end)
    frame:HookScript("OnHide", disarmHousingVisit)
    ...
    frame.houseIcon:SetPoint("TOPLEFT", frame.custom, "BOTTOMLEFT", 0, -8)
    frame.houseText:SetPoint("LEFT", frame.houseIcon, "RIGHT", 5, 0)
...
function MemberTooltip:ShowForPlayer(anchorFrame, player, guildKey, mode)
    ...
    local resolvedName = Nicknames:GetDisplayName(guildKey, player.guid)
    local displayName = resolvedName ~= "" and resolvedName or player.name:gsub("%-.*", "")
    local mainGUID = Alts:GetMain(guildKey, player.guid)
    ...
    if mode == "communities" then
        setFactionIcon(f.factionIcon, GP:GetModule("Roster"):GetFactionForGUID(player.guid))
    else
        f.factionIcon:Hide()
    end
    ...
    if ownership then
        f.houseIcon:Show()
        f.houseText:Show()
        f.houseText:SetText(Housing:FormatPlotOwnership(ownership))
    else
        f.houseIcon:Hide()
        f.houseText:Hide()
    end
    ...
    local housingHeight = f.houseIcon:IsShown() and 22 or 0
    local editorHeight = f.altEditorOpen and 36 or 0
    local contentHeight = math.max(330 + editorHeight, 274 + customHeight + housingHeight + editorHeight, 270 + altRows * 14 + editorHeight)
    ...
    if type(ownership) == "table" and ownership.neighborhoodGUID and ownership.plotID
        ...
        local neighborhoodGUID, houseGUID, plotID = Housing:GetPreparedVisit(guildKey, player.guid)
        if houseGUID then
            C_Timer.After(0, function()
                if frame and frame:IsShown() and activeGuildKey == guildKey
                    and activePlayer and activePlayer.guid == player.guid then
                    armHousingVisit(neighborhoodGUID, houseGUID, plotID)
                end
            end)
        else
            Housing:PrepareVisit(guildKey, player.guid)
        end
    end
end
...
function MemberTooltip:HideSoon(anchorFrame)
    ...
    C_Timer.After(0.12, function()
        ...
        if frame.housingVisitButton and frame.housingVisitButton:IsShown()
            and frame.housingVisitButton:IsMouseOver() then return end
...
MemberTooltip:RegisterMessage("GuildParagon_HousingVisitPrepared", function(_event, guildKey, guid, houseGUID)
    if not houseGUID or not frame or not frame:IsShown() or guildKey ~= activeGuildKey
        or not activePlayer or guid ~= activePlayer.guid then return end
    local Housing = GP:GetModule("Housing", true)
    ...
    C_Timer.After(0, function()
        if frame and frame:IsShown() and guildKey == activeGuildKey
            and activePlayer and guid == activePlayer.guid then
            armHousingVisit(neighborhoodGUID, preparedHouseGUID, plotID)
        end
    end)
end)

local housingCombatFrame = CreateFrame("Frame")
housingCombatFrame:RegisterEvent("PLAYER_REGEN_DISABLED")
housingCombatFrame:RegisterEvent("PLAYER_REGEN_ENABLED")
housingCombatFrame:SetScript("OnEvent", function(_, event)
    if event == "PLAYER_REGEN_DISABLED" then
        housingVisitNeedsDisarm = true
    elseif housingVisitNeedsDisarm then
        disarmHousingVisit()
    end
end)
