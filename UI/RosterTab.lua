...
local birthdayLabel, birthdaySaveButton, birthdayClearButton
...
local function classColorOf(classFile)
    local c = classFile and GP:GetClassColor(classFile)
...
local function nicknameConflictTextFor(player, guildKey)
    local conflict = GP:GetModule("Nicknames"):GetMigrationConflict(guildKey, player.guid)
    if not conflict then return nil end
    local guildData = getGuildData()
    local lines = { GP.L["Conflicting legacy nicknames were preserved. Save a Guild Nickname above to resolve them:"] }
    local entries = {}
    for guid, record in pairs(conflict.members or {}) do
        local member = guildData and ((guildData.roster or {})[guid] or (guildData.formerMembers or {})[guid])
        local name = member and member.name or guid
        local value = record.value ~= "" and record.value or GP.L["(cleared)"]
        local stamp = type(record.updatedAt) == "number" and record.updatedAt > 0
            and GP:DisplayDate("%Y-%m-%d %H:%M", record.updatedAt) or GP.L["Unknown"]
        entries[#entries + 1] = string.format("%s: %s (%s)", name, value, stamp)
    end
    table.sort(entries)
...
function RN:SetFactionIcon(texture, faction)
    local atlas = faction == "Alliance" and "communities-create-button-wow-alliance"
        or faction == "Horde" and "communities-create-button-wow-horde" or nil
    if atlas then
...
function RN:DisarmHousingVisit()
    local button = self.housingVisitButton
    if not button then return end
    GameTooltip_Hide()
    if InCombatLockdown and InCombatLockdown() then
        self.housingVisitNeedsDisarm = true
        return
    end
    self.housingVisitNeedsDisarm = false
...
function RN:ArmHousingVisit(neighborhoodGUID, houseGUID, plotID)
    local button = self.housingVisitButton
    if not button or not neighborhoodGUID or not houseGUID or not plotID then
        self:DisarmHousingVisit()
        return
    end
    if InCombatLockdown and InCombatLockdown() then return end
    self.housingVisitNeedsDisarm = false
    local left, bottom = self.housingDetailButton:GetLeft(), self.housingDetailButton:GetBottom()
    local width, height = self.housingDetailButton:GetWidth(), self.housingDetailButton:GetHeight()
    if not left or not bottom or not width or not height then return end
    local sourceScale = self.housingDetailButton:GetEffectiveScale()
    local targetScale = UIParent:GetEffectiveScale()
    if not sourceScale or not targetScale or targetScale == 0 then return end
...
local function rosterSortValue(player, guildData, guildKey, canSeeOfficerNotes)
    local index = tonumber(sortKey:match("^column(%d)$"))
    if index then
        local columns = GP.UI.RosterColumns
        local key = columns:Source(index, canSeeOfficerNotes)
        local value = columns:Value(player, guildKey, key,
            key == "tag" and displayedTagTextFor(player, guildData, guildKey) or nil)
        if columns:Definition(key).numeric then return value or -1 end
        return normalizedSortText(value)
    end
...
local function createRosterRow(parent)
    ...
    local houseIcon = row:CreateTexture(nil, "ARTWORK")
    houseIcon:SetSize(14, 14)
    houseIcon:SetPoint("LEFT", lastOnline, "RIGHT", COL.gap, 0)
    if houseIcon.SetAtlas then
        houseIcon:SetAtlas("housing-map-plot-player-house")
    else
        houseIcon:SetTexture("Interface\\MINIMAP\\TRACKING\\StableMaster")
    end
    houseIcon:Hide()
    row.houseIcon = houseIcon
    ...
    -- Fixed width; the second note column owns the row's remaining space.
...
local function updateRosterRow(row, player)
    ...
    local showHousingIcon = display.showHousingIcon and Housing and Housing:IsPlotOwner(guildKey, player.guid)
    row.houseIcon:SetShown(showHousingIcon and true or false)
    row.nick:ClearAllPoints()
    if display.showHousingIcon and not GP:IsForeverClient() then
        row.nick:SetPoint("LEFT", row.houseIcon, "RIGHT", COL.gap, 0)
    else
        row.nick:SetPoint("LEFT", row.lastOnline, "RIGHT", COL.gap, 0)
    end
    local columns = GP.UI.RosterColumns
    local canSeeOfficerNotes = GP:GetModule("CustomNotes"):CanAccessOfficerNotes()
    local cells = {row.nick, row.tag, row.note, row.officerNote}
    for index, cell in ipairs(cells) do
        ...
        if key == "nickname" then
            ...
            if conflict then value = value .. " [!]" end
        end
        if key == "mythicScore" and (value == nil or value == 0) then value = "-" end
        cell:SetText(value == nil and "" or tostring(value))
        cell:Show()
    end
end
...
local function buildDisplayList()
    ...
    local canSeeOfficerNotes = CustomNotes:CanAccessOfficerNotes()
    local canSeeLabels = Labels:CanUse()
    local filterText = strtrim(searchBox:GetText() or ""):lower()
    local searchFaction = filterText == "alliance" or filterText == "horde"
    local Roster = searchFaction and GP:GetModule("Roster") or nil
    local auditActive = hasActiveAuditFilters()
    for _, player in pairs(guildData.roster) do
        local matchesSearch = true
        if filterText ~= "" then
            local nickname = Nicknames:GetSharedNickname(guildKey, player.guid) or ""
            local lastOnlineText = formatLastOnlineTime(player)
            local customNote = CustomNotes:Get(guildKey, player.guid) or ""
            local officerNote = canSeeOfficerNotes and (player.officerNote or "") or ""
            local customOfficerNote = canSeeOfficerNotes and (CustomNotes:GetOfficer(guildKey, player.guid) or "") or ""
            local labelNames = canSeeLabels and Labels:GetLabelNamesForPlayer(guildKey, player.guid) or ""
            local ownership = Housing and Housing:GetPlotOwnership(guildKey, player.guid)
            local housingText = ""
            local factionText = searchFaction and (Roster:GetFactionForGUID(player.guid) or "") or ""
            if ownership then
                housingText = "house housing plot subdivision"
                if ownership.subdivision then
                    housingText = housingText .. " subdivision " .. ownership.subdivision .. " subdivision #" .. ownership.subdivision
                end
            end
            local searchable = (player.name .. " " .. (player.rankName or "") .. " " .. (player.note or "") .. " " .. officerNote .. " " .. customNote .. " " .. customOfficerNote .. " " .. nickname .. " " .. lastOnlineText .. " " .. labelNames .. " " .. housingText .. " " .. factionText):lower()
            matchesSearch = searchable:find(filterText, 1, true) ~= nil
        end
        if matchesSearch then
            if (not showOnlineOnly or player.online)
                and (not showUntaggedOnly or not isTagged(guildKey, player.guid)) then
                if not auditActive or matchesActiveAuditFilters(auditIssuesFor(player, guildKey)) then
...
local function hideMemberControls()
    ...
    PA.label:Hide(); PA.box:Hide(); PA.button:Hide()
    ...
    birthdayLabel:Hide(); RN.birthdayPicker:Hide(); birthdaySaveButton:Hide(); birthdayClearButton:Hide()
    ...
    if AL.labelsFlyoutPanel then AL.labelsFlyoutPanel:Hide() end
    if RN.housingDetailButton then RN.housingDetailButton:Hide() end
    if RN.factionDetailIcon then RN.factionDetailIcon:Hide() end
...
local function refreshHistory(guildKey)
    ...
    for i = #log, 1, -1 do
        if log[i].guid == selectedGUID and EventLog:CanDisplayEntry(log[i]) and not EventLog:IsRemoved(guildKey, log[i]) then
            table.insert(lines, GP:DisplayDate("%Y-%m-%d", log[i].ts) .. "  " .. EventLog:Render(GP:DisplayEventDate(log[i])))
...
function RN:RefreshDetailExtras(player, changed)
    if not self.detailExtras then return end
    local extras = self.detailExtras
    for _, item in ipairs(extras.flyouts) do
        item.button:SetShown(player ~= nil)
        if changed or not player then item.panel:Hide() end
        item:ResizeText()
    end
    for _, item in ipairs(extras.notes) do
        local allowed = player ~= nil and (not item.officer or GP:GetModule("CustomNotes"):CanAccessOfficerNotes())
        item.panel:SetShown(allowed)
        item.body:SetText(allowed and (GP:SafeOptionalString(player[item.key]) or "") or "")
    end
end

local function refreshDetail()
    if RN.rankControl then RN.rankControl:Refresh() end
    local L = GP.L
    local guildData, guildKey = getGuildData()
    ...
    if not player then
        ...
        hideMemberControls()
        RN:RefreshDetailExtras(nil, true)
    ...
    RN:SetFactionIcon(RN.factionDetailIcon, GP:GetModule("Roster"):GetFactionForGUID(player.guid))
    ...
    PA.label:Show(); PA.box:Show(); PA.button:Show()
    customNoteLabel:Show(); customNoteBox:Show(); customNoteButton:Show()
    birthdayLabel:Show(); RN.birthdayPicker:Show(); birthdaySaveButton:Show(); birthdayClearButton:Show()
    auditDetailHeading:Show(); auditDetailBody:Show()
    JD.label:Show(); JD.box:Show(); JD.sourceText:Show()
    if isNewSelection then
        local Nicknames = GP:GetModule("Nicknames")
        nickBox:SetText(Nicknames:GetSharedNickname(guildKey, selectedGUID))
        PA.box:SetText(Nicknames:GetPersonalAlias(guildKey, selectedGUID))
        customNoteBox:SetText(singleLine(GP:GetModule("CustomNotes"):Get(guildKey, selectedGUID)))
        local day, month = GP:GetModule("Roster"):GetBirthday(player)
        RN.birthdayPicker:SetDate(day, month)
        JD.box:SetText(GP:DisplayDate("%Y-%m-%d", player.firstSeen))
    end
    ...
    RN.birthdayPicker:SetEditable(canEditProfile)
    ...
    local Housing = GP:GetModule("Housing", true)
    local ownership = Housing and Housing:GetPlotOwnership(guildKey, selectedGUID)
    if ownership then
        RN.housingDetailButton:Show()
        RN.housingDetailButton.ownership = ownership
        RN.housingDetailText:SetText(Housing:FormatPlotOwnership(ownership))
        local neighborhoodGUID, houseGUID, plotID = Housing:GetPreparedVisit(guildKey, selectedGUID)
        if houseGUID then
            RN:ArmHousingVisit(neighborhoodGUID, houseGUID, plotID)
        else
            RN:DisarmHousingVisit()
            if ownership.neighborhoodGUID and ownership.plotID then
                local _, message = Housing:PrepareVisit(guildKey, selectedGUID)
                if message == L["Preparing housing visit…"] then setStatus(message, false) end
            end
        end
    else
        RN.housingDetailButton:Hide()
        RN.housingDetailButton.ownership = nil
        RN:DisarmHousingVisit()
    end

    local conflictText = nicknameConflictTextFor(player, guildKey)
    auditDetailHeading:SetText(conflictText and L["Nickname Review Needed"] or L["Audit Details"])
    auditDetailBody:SetText(conflictText or auditDetailTextFor(player, guildKey))
    refreshHistory(guildKey)
    RN:RefreshDetailExtras(player, isNewSelection)
end
...
local pendingRefreshRequests, pendingRefreshTrigger = 0, 0
local refreshTriggers = {GuildParagon_RosterScanned=1, GuildParagon_AltsChanged=2,
    GuildParagon_NicknamesChanged=3, GuildParagon_CustomNotesChanged=4,
    GuildParagon_JoinDateChanged=5, GuildParagon_BirthdayChanged=6,
    GuildParagon_LogEntryAdded=7, GuildParagon_LabelsChanged=8,
    GuildParagon_RosterDisplaySettingsChanged=9, GuildParagon_HousingChanged=10}
function RosterTab:Refresh(resetScroll)
    GP:MonitorDetail("requests", pendingRefreshRequests)
    GP:MonitorDetail("trigger", pendingRefreshTrigger)
    pendingRefreshRequests, pendingRefreshTrigger = 0, 0
    if not frame then return end

    if RN.columnHeaders then
        local columns = GP.UI.RosterColumns
        local allowed = GP:GetModule("CustomNotes"):CanAccessOfficerNotes()
        for index, header in ipairs(RN.columnHeaders) do
            header.label = columns:Label(columns:Source(index, allowed))
            header.text:SetText(header.label .. (sortKey == "column" .. index and (sortAsc and " ^" or " v") or ""))
        end
    end

    if RN.nickHeader and RN.lastOnlineHeader then
        RN.nickHeader:ClearAllPoints()
        if rosterDisplaySettings().showHousingIcon and not GP:IsForeverClient() then
            RN.nickHeader:SetPoint("LEFT", RN.lastOnlineHeader, "RIGHT", COL.gap + COL.houseWidth + COL.gap, 0)
        else
            RN.nickHeader:SetPoint("LEFT", RN.lastOnlineHeader, "RIGHT", COL.gap, 0)
        end
    end

    local guildData, guildKey = getGuildData()
    local Housing = GP:GetModule("Housing", true)
    if guildKey and Housing and Housing.MaybeAutoScan then
        Housing:MaybeAutoScan(guildKey)
    end
    local displayList = GP:MonitorCall("ui.rosterList", buildDisplayList)
    GP:MonitorDetail("records", #displayList)
    GP:MonitorCall("ui.rosterRows", list.SetData, list, displayList, resetScroll)
    ...
    GP:MonitorCall("ui.rosterDetail", refreshDetail)
...
function RN:BuildDetailExtras(parent, macroButton)
    local extras = {flyouts={}, notes={}}
    self.detailExtras = extras
    local previous = macroButton
    for _, label in ipairs({"Audit Details", "Recent History"}) do
        local item = {}
        item.button = Theme:CreateButton(parent, GP.L[label])
        item.button:SetWidth(132)
        item.button:SetPoint("RIGHT", previous, "LEFT", -8, 0)
        previous = item.button
        item.panel = Theme:CreatePanel(parent, "panelRaised", "accent")
        item.panel:SetFrameStrata("DIALOG")
        item.panel:SetSize(560, 360)
        item.panel:SetClampedToScreen(true)
        item.panel:EnableMouse(true)
        item.panel:SetPoint("BOTTOMRIGHT", item.button, "TOPRIGHT", 0, 4)
        item.heading = item.panel:CreateFontString(nil, "ARTWORK")
        item.heading:SetFontObject(Theme.font.heading)
        item.heading:SetPoint("TOPLEFT", 12, -12)
        item.heading:SetText(GP.L[label])
        local close = Theme:CreateButton(item.panel, GP.L["Close"])
        close:SetPoint("TOPRIGHT", -8, -8)
        close:SetScript("OnClick", function() item.panel:Hide() end)
        local scroll = CreateFrame("ScrollFrame", nil, item.panel)
        scroll:SetPoint("TOPLEFT", 12, -44)
        scroll:SetPoint("BOTTOMRIGHT", -26, 12)
        item.content = CreateFrame("Frame", nil, scroll)
        item.content:SetSize(522, 1)
        scroll:SetScrollChild(item.content)
        item.body = item.content:CreateFontString(nil, "ARTWORK")
        item.body:SetFontObject(Theme.font.body)
        item.body:SetPoint("TOPLEFT")
        item.body:SetWidth(522)
        item.body:SetWordWrap(true)
        item.body:SetNonSpaceWrap(true)
        item.body:SetJustifyH("LEFT")
        item.body:SetJustifyV("TOP")
        local track = Theme:CreatePanel(item.panel, "panel", "border")
        track:SetPoint("TOPRIGHT", -12, -44)
        ...
        thumb:SetColorTexture(unpack(Theme.color.accentDim))
        thumb:SetSize(6, 24)
        slider:SetThumbTexture(thumb)
        slider:SetScript("OnValueChanged", function(_, value) scroll:SetVerticalScroll(value) end)
        function item:ResizeText()
            -- Leave the font string height unconstrained so every wrapped line is measured.
            self.body:SetHeight(0)
            local height = math.max(1, self.body:GetStringHeight())
            self.content:SetHeight(height)
            local visible = math.max(1, scroll:GetHeight())
            local maximum = math.max(0, height - visible)
            slider:SetMinMaxValues(0, maximum)
            slider:SetValue(math.min(slider:GetValue() or 0, maximum))
            thumb:SetHeight(math.max(24, math.min(visible, visible * visible / height)))
            track:SetShown(maximum > 0)
        end
        scroll:EnableMouseWheel(true)
        scroll:SetScript("OnMouseWheel", function(_, delta)
            local _, maximum = slider:GetMinMaxValues()
            slider:SetValue(math.max(0, math.min(maximum, slider:GetValue() - delta * 30)))
        end)
        item.button:SetScript("OnClick", function()
            local opening = not item.panel:IsShown()
            for _, other in ipairs(extras.flyouts) do other.panel:Hide() end
            item.panel:SetShown(opening)
            slider:SetValue(0)
            scroll:SetVerticalScroll(0)
        end)
        item.panel:SetScript("OnShow", function(self)
            self.mouseDown = IsMouseButtonDown("LeftButton")
            item:ResizeText()
        end)
        item.panel:SetScript("OnUpdate", function(self)
            local down = IsMouseButtonDown("LeftButton")
            if down and not self.mouseDown and not self:IsMouseOver() and not item.button:IsMouseOver() then self:Hide() end
            self.mouseDown = down
        end)
        item.panel:Hide()
        table.insert(extras.flyouts, item)
    end
    for i, spec in ipairs({{key="note", label="Blizzard Guild Note"}, {key="officerNote", label="Blizzard Officer Note", officer=true}}) do
        local item = {key=spec.key, officer=spec.officer}
        item.panel = Theme:CreatePanel(parent, "panelRaised", "border")
        item.panel:SetPoint("TOPLEFT", parent, "TOPLEFT", 560, -12-(i-1)*58)
        item.panel:SetPoint("TOPRIGHT", parent, "TOPRIGHT", -12, -12-(i-1)*58)
        item.panel:SetHeight(52)
        local title = item.panel:CreateFontString(nil, "ARTWORK")
        title:SetFontObject(Theme.font.small)
        title:SetPoint("TOPLEFT", 10, -8)
        title:SetText(GP.L[spec.label])
        item.body = item.panel:CreateFontString(nil, "ARTWORK")
        item.body:SetFontObject(Theme.font.body)
        item.body:SetPoint("TOPLEFT", 10, -23)
        item.body:SetPoint("BOTTOMRIGHT", -10, 5)
        item.body:SetWordWrap(true)
        item.body:SetJustifyH("LEFT")
        item.body:SetJustifyV("TOP")
        table.insert(extras.notes, item)
    end
    for i, controls in ipairs({
        {customNoteLabel, customNoteBox, customNoteButton},
        {officerNoteLabel, officerNoteBox, officerNoteButton},
    }) do
        local label, box, save = unpack(controls)
        local y = -130 - (i-1)*58
        label:ClearAllPoints()
        label:SetPoint("TOPLEFT", parent, "TOPLEFT", 570, y)
        box:ClearAllPoints()
        box:SetWidth(0)
        box:SetPoint("TOPLEFT", parent, "TOPLEFT", 570, y-17)
        box:SetPoint("TOPRIGHT", parent, "TOPRIGHT", -84, y-17)
        save:ClearAllPoints()
        save:SetPoint("LEFT", box, "RIGHT", 8, 0)
    end
    officerNoteLockedText:ClearAllPoints()
    officerNoteLockedText:SetPoint("TOPLEFT", officerNoteLabel, "BOTTOMLEFT", 0, -8)
    parent:HookScript("OnHide", function() self:RefreshDetailExtras(nil, true) end)
    return extras.flyouts[2].heading, extras.flyouts[2].body, extras.flyouts[1].heading, extras.flyouts[1].body
end
...
local function buildAltsLabelsFlyouts(parent)
    ...
    -- Same row (FIRST_ROW_Y - ROW_STRIDE*3, see refreshDetail) as
    ...
    AL.labelsCountText:SetPoint("TOPLEFT", detailHeading, "BOTTOMLEFT", LABELS_COLUMN_X, FIRST_ROW_Y - ROW_STRIDE * 3)
    AL.labelsCountText:Hide()
...
local function buildDetailPanel(parent)
    ...
    RN.factionDetailIcon = detail:CreateTexture(nil, "ARTWORK")
    RN.factionDetailIcon:SetSize(18, 18)
    RN.factionDetailIcon:SetPoint("LEFT", detailHeading, "RIGHT", 5, 0)
    RN.factionDetailIcon:Hide()

    detailLastOnlineText = detail:CreateFontString(nil, "ARTWORK")
    detailLastOnlineText:SetFontObject(Theme.font.muted)
    detailLastOnlineText:SetPoint("LEFT", detailHeading, "RIGHT", 30, 0)
    ...
    PA.label = detail:CreateFontString(nil, "ARTWORK")
    PA.label:SetFontObject(Theme.font.small)
    PA.label:SetPoint("LEFT", nickButton, "RIGHT", 18, 0)
    PA.label:SetText(L["My Alias:"])

    PA.box = Theme:CreateEditBox(detail, 120)
    PA.box:SetPoint("LEFT", PA.label, "RIGHT", 8, 0)

    PA.button = Theme:CreateButton(detail, L["Save"])
    PA.button:SetPoint("LEFT", PA.box, "RIGHT", 8, 0)
    PA.button:SetScript("OnClick", function()
        local _, guildKey = getGuildData()
        local ok, err = GP:GetModule("Nicknames"):SetPersonalAlias(guildKey, selectedGUID, PA.box:GetText())
        if ok then
            setStatus(L["Personal alias saved locally."])
            RosterTab:Refresh()
        else
            setStatus(err, true)
        end
    ...
    birthdayLabel:SetPoint("TOPLEFT", detailHeading, "BOTTOMLEFT", 0, FIRST_ROW_Y - ROW_STRIDE * 1)
    birthdayLabel:SetText(L["Birthday:"])

    RN.birthdayPicker = GP.UI.BirthdayPicker:New(detail)
    RN.birthdayPicker:SetPoint("LEFT", birthdayLabel, "RIGHT", 8, 0)

    birthdaySaveButton = Theme:CreateButton(detail, L["Save"])
    birthdaySaveButton:SetPoint("LEFT", RN.birthdayPicker, "RIGHT", 8, 0)
    birthdaySaveButton:SetScript("OnClick", function()
        local _, guildKey = getGuildData()
        local day, month = RN.birthdayPicker:GetDate()
        local ok, err = GP:GetModule("Roster"):SetBirthday(guildKey, selectedGUID, day, month)
        if ok then
            setStatus(L["Saved."])
            RosterTab:Refresh()
        else
    ...
    birthdayClearButton:SetScript("OnClick", function()
        ...
        if ok then
            RN.birthdayPicker:SetDate(nil, nil)
            setStatus(L["Cleared."])
            RosterTab:Refresh()
        else
            setStatus(err, true)
    ...
    RN.rankControl = GP.UI.RosterRankControl:New(detail, birthdayClearButton, function()
        local guildData = getGuildData()
        local member = guildData and selectedGUID and (guildData.roster[selectedGUID] or guildData.formerMembers[selectedGUID])
        return selectedGUID, member and member.rankName, guildData and selectedGUID and guildData.roster[selectedGUID] ~= nil
    end)
    RosterTab:RegisterMessage("GuildParagon_RankAccessChanged", function() RN.rankControl:Hide(); RN.rankControl:Refresh() end)
    ...
    JD.label:SetPoint("TOPLEFT", detailHeading, "BOTTOMLEFT", 0, FIRST_ROW_Y - ROW_STRIDE * 2)
    JD.label:SetText(L["Joined:"])

    JD.box = Theme:CreateEditBox(detail, 100)
    JD.box:SetPoint("LEFT", JD.label, "RIGHT", 8, 0)
    JD.box:HookScript("OnEnter", function(self)
        GameTooltip:SetOwner(self, "ANCHOR_TOP")
        GameTooltip:SetText(string.format(L["Enter the join date as %s."], GP:JoinDateInputFormat()))
        GameTooltip:Show()
    end)
    JD.box:HookScript("OnLeave", function() GameTooltip:Hide() end)
    ...
    JD.saveButton:SetScript("OnClick", function()
        ...
        local newDate = GP:CanonicalJoinDateInput(JD.box:GetText())
        newDate = newDate and canonicalJoinDateText(newDate)
        if not newDate then
            setStatus(string.format(L["Enter the join date as %s."], GP:JoinDateInputFormat()), true)
            return
        end
        local targetGUID = selectedGUID
        local function doSave()
            local ok, err = GP:GetModule("Roster"):SetJoinDate(guildKey, targetGUID, newDate)
    ...
    RN.housingDetailButton = CreateFrame("Button", nil, detail)
    RN.housingDetailButton:SetSize(190, 22)
    RN.housingDetailButton:SetPoint("TOPLEFT", detailHeading, "BOTTOMLEFT", 0, FIRST_ROW_Y - ROW_STRIDE * 5)
    RN.housingDetailButton:Hide()

    RN.housingDetailIcon = RN.housingDetailButton:CreateTexture(nil, "ARTWORK")
    RN.housingDetailIcon:SetSize(14, 14)
    RN.housingDetailIcon:SetPoint("LEFT", 0, 0)
    if RN.housingDetailIcon.SetAtlas then
        RN.housingDetailIcon:SetAtlas("housing-map-plot-player-house")
    else
        RN.housingDetailIcon:SetTexture("Interface\\MINIMAP\\TRACKING\\StableMaster")
    end

    RN.housingDetailText = RN.housingDetailButton:CreateFontString(nil, "ARTWORK")
    RN.housingDetailText:SetFontObject(Theme.font.body)
    RN.housingDetailText:SetTextColor(unpack(Theme.color.accent))
    RN.housingDetailText:SetPoint("LEFT", RN.housingDetailIcon, "RIGHT", 6, 0)
    RN.housingDetailText:SetWidth(160)
    RN.housingDetailText:SetJustifyH("LEFT")
    RN.housingDetailText:SetWordWrap(false)
    local function showHousingTooltip(self)
        RN.housingDetailText:SetTextColor(unpack(Theme.color.warning))
        GameTooltip:SetOwner(self, "ANCHOR_TOPLEFT")
        GameTooltip:SetText(L["Subdivision"])
        GameTooltip:AddLine(L["Selecting this member prepares the secure visit; click the address when ready."], nil, nil, nil, true)
        GameTooltip:Show()
    end
    local function hideHousingTooltip()
        RN.housingDetailText:SetTextColor(unpack(Theme.color.accent))
        GameTooltip_Hide()
    end
    RN.housingDetailButton:SetScript("OnEnter", showHousingTooltip)
    RN.housingDetailButton:SetScript("OnLeave", hideHousingTooltip)
    RN.housingDetailButton:SetScript("OnClick", function()
        local _, currentGuildKey = getGuildData()
        local ok, message = GP:GetModule("Housing"):PrepareVisit(currentGuildKey, selectedGUID)
        setStatus(message, not ok and message ~= L["Preparing housing visit…"])
    end)

    -- This protected button is isolated under UIParent so protection does not
    -- propagate into Guild Paragon's window hierarchy. It is only positioned
    -- and armed outside combat, then WoW's secure handler performs the visit.
    RN.housingVisitButton = CreateFrame("Button", nil, UIParent, "SecureActionButtonTemplate")
    RN.housingVisitButton:SetFrameStrata("TOOLTIP")
    RN.housingVisitButton:SetFrameLevel(100)
    -- Support either action-button click phase selected by the client.
    RN.housingVisitButton:RegisterForClicks("AnyDown", "AnyUp")
    RN.housingVisitButton:Hide()
    RN.housingVisitButton:SetScript("OnEnter", showHousingTooltip)
    RN.housingVisitButton:SetScript("OnLeave", hideHousingTooltip)
    RN.housingVisitButton:SetScript("PostClick", function()
        setStatus(L["Housing visit action pressed."], false)
        RN:DisarmHousingVisit()
    end)
    ...
    statusText:SetPoint("TOPLEFT", detailHeading, "BOTTOMLEFT", 0, FIRST_ROW_Y - ROW_STRIDE * 6)
    statusText:SetPoint("RIGHT", detail, "LEFT", 540, 0)
    statusText:SetJustifyH("LEFT")

    historyHeading, historyBody, auditDetailHeading, auditDetailBody = RN:BuildDetailExtras(detail, macroRulesButton)
...
function RosterTab:Build(parent)
    ...
    RN.guildInviteButton=GP.UI.GuildInvite:CreateButton(frame,untaggedOnlyButton)
    ...
    summaryText:SetPoint("LEFT", RN.guildInviteButton, "RIGHT", 12, 0)
    ...
    RN.lastOnlineHeader = addHeaderButton(headerRow, L["Last Online"], "lastOnline", COL.lastWidth)
    RN.lastOnlineHeader:SetPoint("LEFT", levelHeader, "RIGHT", COL.gap, 0)
    RN.nickHeader = addHeaderButton(headerRow, L["Nickname"], "column1", COL.nickWidth)
    RN.nickHeader:SetPoint("LEFT", RN.lastOnlineHeader, "RIGHT", COL.gap, 0)
    local tagHeader = addHeaderButton(headerRow, L["Alt / Main"], "column2", COL.tagWidth)
    tagHeader:SetPoint("LEFT", RN.nickHeader, "RIGHT", COL.gap, 0)
    RN.noteHeader = addHeaderButton(headerRow, L["Note"], "column3", NOTE_COL_WIDTH)
    RN.noteHeader:SetPoint("LEFT", tagHeader, "RIGHT", COL.gap, 0)
    RN.officerNoteHeader = addHeaderButton(headerRow, L["Officer Note"], "column4", OFFICER_NOTE_COL_WIDTH)
    RN.officerNoteHeader:SetPoint("LEFT", RN.noteHeader, "RIGHT", COL.gap, 0)

    RN.columnHeaders = {RN.nickHeader, tagHeader, RN.noteHeader, RN.officerNoteHeader}
    setSortIndicators()
    ...
    local function debouncedRefresh(event)
        pendingRefreshTrigger = not GP:IsSecretValue(event) and refreshTriggers[event] or 0
        pendingRefreshRequests = math.min(100000000, pendingRefreshRequests + 1)
    ...
    RosterTab:RegisterMessage("GuildParagon_RosterScanned", GP:MonitorWrap("listener.rosterUI", debouncedRefresh))
    ...
    RosterTab:RegisterMessage("GuildParagon_RosterDisplaySettingsChanged", debouncedRefresh)
    RosterTab:RegisterMessage("GuildParagon_GuildHealthSettingsChanged", debouncedRefresh)
    RosterTab:RegisterMessage("GuildParagon_HousingChanged", debouncedRefresh)
    RosterTab:RegisterMessage("GuildParagon_HousingVisitPrepared", function(_event, guildKey, guid, houseGUID, message)
        local _, activeGuildKey = getGuildData()
        if guildKey == activeGuildKey and guid == selectedGUID then
            setStatus(message, not houseGUID)
            debouncedRefresh()
        end
    end)
    RosterTab:RegisterMessage("GuildParagon_HousingScanUpdated", function()
        debouncedRefresh()
    end)
    ...
    RosterTab:RegisterEvent("PLAYER_REGEN_ENABLED", function()
        if RN.housingVisitNeedsDisarm then
            RN:DisarmHousingVisit()
        end
        if frame and frame:IsShown() then
            debouncedRefresh()
