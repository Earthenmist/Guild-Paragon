...
local NOTE_COLUMN_SOURCES = {
    note = {
        label = function() return GP.L["Note"] end,
        maxChars = NOTE_MAX_CHARS,
        officerOnly = false,
        value = function(player)
            return player and player.note or ""
        end,
    },
    officerNote = {
        label = function() return GP.L["Officer Note"] end,
        maxChars = OFFICER_NOTE_MAX_CHARS,
        officerOnly = true,
        value = function(player)
            return player and player.officerNote or ""
        end,
    },
    customNote = {
        label = function() return GP.L["Custom Note"] end,
        maxChars = NOTE_MAX_CHARS,
        officerOnly = false,
        value = function(player, guildKey)
            return guildKey and GP:GetModule("CustomNotes"):Get(guildKey, player.guid) or ""
        end,
    },
    customOfficerNote = {
        label = function() return GP.L["Custom Officer Note"] end,
        maxChars = OFFICER_NOTE_MAX_CHARS,
        officerOnly = true,
        value = function(player, guildKey)
            return guildKey and GP:GetModule("CustomNotes"):GetOfficer(guildKey, player.guid) or ""
        end,
    },
}

local function validNoteColumnSource(key)
    return NOTE_COLUMN_SOURCES[key] and key or nil
end

local function resolveRosterNoteColumns(canSeeOfficerNotes)
    local display = rosterDisplaySettings()
    local first = validNoteColumnSource(display.noteColumn1) or "note"
    local second = validNoteColumnSource(display.noteColumn2) or "officerNote"
    if first == second then second = (first == "note") and "customNote" or "note" end

    if not canSeeOfficerNotes then
        if NOTE_COLUMN_SOURCES[first].officerOnly then first = "note" end
        if NOTE_COLUMN_SOURCES[second].officerOnly then second = "customNote" end
        if first == second then second = (first == "note") and "customNote" or "note" end
    end
...
local function noteColumnValue(player, guildKey, sourceKey)
    local source = NOTE_COLUMN_SOURCES[sourceKey] or NOTE_COLUMN_SOURCES.note
    return source.value(player, guildKey) or ""
end

local function noteColumnLabel(sourceKey)
    local source = NOTE_COLUMN_SOURCES[sourceKey] or NOTE_COLUMN_SOURCES.note
    return source.label()
end
...
local function nicknameConflictTextFor(player, guildKey)
    ...
    for guid, record in pairs(conflict.members or {}) do
        ...
        local stamp = type(record.updatedAt) == "number" and record.updatedAt > 0
            and date("%Y-%m-%d %H:%M", record.updatedAt) or GP.L["Unknown"]
...
local function rosterSortValue(player, guildData, guildKey, canSeeOfficerNotes)
    if sortKey == "nickname" then
        return normalizedSortText(GP:GetModule("Nicknames"):GetSharedNickname(guildKey, player.guid))
    end
    if sortKey == "tag" then
        return normalizedSortText(displayedTagTextFor(player, guildData, guildKey))
    end
    if sortKey == "noteColumn1" or sortKey == "noteColumn2" then
        local column1, column2 = resolveRosterNoteColumns(canSeeOfficerNotes)
        return normalizedSortText(noteColumnValue(player, guildKey, sortKey == "noteColumn1" and column1 or column2))
    end
...
local function updateRosterRow(row, player)
    ...
    local nickname = guildKey and GP:GetModule("Nicknames"):GetSharedNickname(guildKey, player.guid) or ""
    ...
    if display.showHousingIcon then
        row.nick:SetPoint("LEFT", row.houseIcon, "RIGHT", COL.gap, 0)
    else
        row.nick:SetPoint("LEFT", row.lastOnline, "RIGHT", COL.gap, 0)
    end
    local conflict = guildKey and GP:GetModule("Nicknames"):GetMigrationConflict(guildKey, player.guid)
    local nickText = nickname ~= "" and truncate("\"" .. nickname .. "\"", NICK_MAX_CHARS) or ""
    row.nick:SetText(conflict and (nickText .. " [!]") or nickText)
    ...
    local column1, column2 = resolveRosterNoteColumns(canSeeOfficerNotes)
    row.note:SetText(truncate(noteColumnValue(player, guildKey, column1), NOTE_COLUMN_SOURCES[column1].maxChars))
    row.officerNote:Show()
    row.officerNote:SetText(truncate(noteColumnValue(player, guildKey, column2), NOTE_COLUMN_SOURCES[column2].maxChars))
end
...
function RosterTab:Refresh(resetScroll)
    ...
    if RN.noteHeader and RN.officerNoteHeader then
        local column1, column2 = resolveRosterNoteColumns(GP:GetModule("CustomNotes"):CanAccessOfficerNotes())
        RN.noteHeader.label = noteColumnLabel(column1)
        RN.officerNoteHeader.label = noteColumnLabel(column2)
        RN.noteHeader.text:SetText(RN.noteHeader.label .. (sortKey == "noteColumn1" and (sortAsc and " ^" or " v") or ""))
        RN.officerNoteHeader.text:SetText(RN.officerNoteHeader.label .. (sortKey == "noteColumn2" and (sortAsc and " ^" or " v") or ""))
    end

    if RN.nickHeader and RN.lastOnlineHeader then
        RN.nickHeader:ClearAllPoints()
        if rosterDisplaySettings().showHousingIcon then
...
local function buildDetailPanel(parent)
    ...
    RN.housingDetailButton:SetPoint("TOPLEFT", detailHeading, "BOTTOMLEFT", 0, FIRST_ROW_Y - ROW_STRIDE * 7)
    RN.housingDetailButton:Hide()
    ...
    statusText:SetPoint("TOPLEFT", detailHeading, "BOTTOMLEFT", 0, FIRST_ROW_Y - ROW_STRIDE * 8)
...
function RosterTab:Build(parent)
    ...
    RN.nickHeader = addHeaderButton(headerRow, L["Nickname"], "nickname", COL.nickWidth)
    RN.nickHeader:SetPoint("LEFT", RN.lastOnlineHeader, "RIGHT", COL.gap, 0)
    ...
    RN.noteHeader = addHeaderButton(headerRow, L["Note"], "noteColumn1", NOTE_COL_WIDTH)
    RN.noteHeader:SetPoint("LEFT", tagHeader, "RIGHT", COL.gap, 0)
    RN.officerNoteHeader = addHeaderButton(headerRow, L["Officer Note"], "noteColumn2", OFFICER_NOTE_COL_WIDTH)
