...
function GP.UI.GuildChatTab:Build(parent)
    ...
    history:SetScript("OnHyperlinkClick",function(self,link,text,button)
        if button~="LeftButton" and button~="RightButton" then return end
        if GP:IsSecretValue(link) or type(link)~="string" then return end
        if Chat:PrivacyEnabled() and not privacyRevealed then return end
        if not Chat:Allowed(club,stream,kind) then return end
        for _,record in ipairs(records) do
            if record.link==link then
                if button=="LeftButton" then Chat:OpenWhisper(club,stream,kind,record) end
                return
            end
        end
        local linkType=link:match("^([^:]+):")
        if (linkType=="item" or linkType=="keystone") and not GP:IsSecretValue(text)
            and type(text)=="string" and type(SetItemRef)=="function" then
            -- Let Blizzard open its item reference tooltip and handle modifiers.
