...
local ROSTER_NOTE_COLUMN_OPTIONS = {
    { key = "note", labelKey = "Note" },
    { key = "officerNote", labelKey = "Officer Note" },
    { key = "customNote", labelKey = "Custom Note" },
    { key = "customOfficerNote", labelKey = "Custom Officer Note" },
}
...
local function createRosterNoteDropdown(parent, width, onSelect)
    ...
    function button:Rebuild(disabledKey)
        ...
        for index, option in ipairs(ROSTER_NOTE_COLUMN_OPTIONS) do
            ...
            row:SetEnabled(option.key ~= disabledKey)
            row:SetAlpha(option.key == disabledKey and 0.45 or 1)
            row:SetScript("OnClick", function()
                if option.key == disabledKey then return end
                hidePanel()
                onSelect(option.key)
            end)
            row:Show()
...
local function buildRosterDisplaySection(frame, generalPage, leftTop, rosterSettings)
    ...
    rosterDisplay:SetSize(SETTINGS_COLUMN_WIDTH, 390)
    ...
    local note1Label = rosterDisplay:CreateFontString(nil, "ARTWORK")
    note1Label:SetFontObject(Theme.font.body)
    note1Label:SetPoint("TOPLEFT", levelDesc, "BOTTOMLEFT", 0, -12)
    note1Label:SetText(L["Roster note column 1:"])

    local note1Dropdown, note2Dropdown
    note1Dropdown = createRosterNoteDropdown(rosterDisplay, 180, function(value)
        rosterSettings.noteColumn1 = value
        if rosterSettings.noteColumn2 == value then
            rosterSettings.noteColumn2 = (value == "note") and "customNote" or "note"
        end
        note1Dropdown:SetValue(rosterSettings.noteColumn1)
        note2Dropdown:SetValue(rosterSettings.noteColumn2)
        note1Dropdown.disabledKey = rosterSettings.noteColumn2
        note2Dropdown.disabledKey = rosterSettings.noteColumn1
        GP:SendMessage("GuildParagon_RosterDisplaySettingsChanged")
    end)
    note1Dropdown:SetPoint("LEFT", note1Label, "RIGHT", 10, 0)

    local note2Label = rosterDisplay:CreateFontString(nil, "ARTWORK")
    note2Label:SetFontObject(Theme.font.body)
    note2Label:SetPoint("TOPLEFT", note1Label, "BOTTOMLEFT", 0, -28)
    note2Label:SetText(L["Roster note column 2:"])

    note2Dropdown = createRosterNoteDropdown(rosterDisplay, 180, function(value)
        rosterSettings.noteColumn2 = value
        if rosterSettings.noteColumn1 == value then
            rosterSettings.noteColumn1 = (value == "note") and "customNote" or "note"
        end
        note1Dropdown:SetValue(rosterSettings.noteColumn1)
        note2Dropdown:SetValue(rosterSettings.noteColumn2)
        note1Dropdown.disabledKey = rosterSettings.noteColumn2
    ...
    note2Dropdown:SetPoint("LEFT", note1Dropdown, "LEFT", 0, -36)
    ...
    local noteColumnDesc = addOptionDescription(rosterDisplay, note2Dropdown, L["Non-officers automatically see Note and Custom Note when an officer-only source is selected."], 380)
    noteColumnDesc:ClearAllPoints()
    noteColumnDesc:SetPoint("TOPLEFT", note2Label, "BOTTOMLEFT", 0, -22)
