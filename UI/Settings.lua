...
local COMPANION_PAGE_WIDTH = 1050
local COMPANION_PAGE_HEIGHT = 630
local COMPANION_PREVIEW_ROW_HEIGHT = 28
...
local function ensureRosterDisplaySettings()
    ...
    if s.showHousingIcon == nil then s.showHousingIcon = false end
    if s.noteColumn1 == nil then s.noteColumn1 = "note" end
    if s.noteColumn2 == nil then s.noteColumn2 = "officerNote" end
    if s.noteColumn1 == s.noteColumn2 then s.noteColumn2 = (s.noteColumn1 == "note") and "customNote" or "note" end
    return s
end

local ROSTER_NOTE_COLUMN_OPTIONS = GP.UI.RosterColumns:Options()

local function rosterNoteColumnLabel(key)
    for _, option in ipairs(ROSTER_NOTE_COLUMN_OPTIONS) do
        if option.key == key then return GP.L[option.labelKey] end
    end
    return GP.L["Note"]
end

local function createRosterNoteDropdown(parent, width, onSelect)
    ...
    function button:SetValue(value)
        self.value = value
        self.text:SetText(rosterNoteColumnLabel(value) .. "  v")
    end

    function button:Rebuild()
        for _, row in ipairs(self.rows) do row:Hide() end
        panel:SetHeight(#ROSTER_NOTE_COLUMN_OPTIONS * 26 + 8)
        for index, option in ipairs(ROSTER_NOTE_COLUMN_OPTIONS) do
            ...
            row:SetPoint("RIGHT", -4, 0)
            row.text:SetText(GP.L[option.labelKey])
            local available = not self.canSelect or self.canSelect(option.key)
            row:SetEnabled(available)
            row:SetAlpha(available and 1 or 0.45)
            row:SetScript("OnClick", function()
                if self.canSelect and not self.canSelect(option.key) then return end
                hidePanel()
                onSelect(option.key)
            end)
            row:Show()
    ...
    button:SetScript("OnHide", hidePanel)
    button:SetValue("note")
    return button
end
...
function GH.updateMacroIgnoreRow(row, record)
    ...
    row.updated:SetText(record.updatedAt and GP:DisplayDate("%Y-%m-%d", record.updatedAt) or GP.L["Never"])
end
...
local function refreshPerformanceText(frame)
    if UpdateAddOnMemoryUsage then GP:MonitorCall("diagnostic.memoryRefresh", UpdateAddOnMemoryUsage) end
...
local function formatTimestamp(ts)
    ts = tonumber(ts)
    if not ts or ts <= 0 then return GP.L["never"] end
    return GP:DisplayDate("%Y-%m-%d %H:%M", ts)
end
...
local function buildHealthSettingsPage(frame, healthPage)
    ...
    if GP:IsForeverClient() then
        mythicSeasonLabel:Hide()
        mythicSeasonDesc:Hide()
        mythicSeasonBox:Hide()
    end
    ...
    local function saveHealthSettings()
        ...
        local saved = GuildHealth:SaveSettings({
            ...
            mythicSeasonStartDate = not GP:IsForeverClient() and mythicSeasonBox:GetText() or nil,
    ...
    saveHealthButton:SetPoint("TOPLEFT", GP:IsForeverClient() and goneDaysBox or mythicSeasonBox, "BOTTOMLEFT", 0, -14)
...
local function buildRosterDisplaySection(frame, generalPage, leftTop, rosterSettings)
    local L = GP.L
    ...
    rosterDisplay:SetSize(SETTINGS_COLUMN_WIDTH, 500)
    ...
    local columnSettings = GP.UI.RosterColumns:Settings()
    local dropdowns = {}
    local previous = levelDesc
    for index=1,4 do
        local label = rosterDisplay:CreateFontString(nil, "ARTWORK")
        label:SetFontObject(Theme.font.body)
        label:SetPoint("TOPLEFT", previous, "BOTTOMLEFT", 0, index == 1 and -12 or -24)
        label:SetText(string.format(L["Roster column %d:"], index))
        local dropdown = createRosterNoteDropdown(rosterDisplay, 180, function(value)
            if not GP.UI.RosterColumns:CanSelect(index, value) then return end
            columnSettings.columns[index] = value
            dropdowns[index]:SetValue(value)
            GP:SendMessage("GuildParagon_RosterDisplaySettingsChanged")
        end)
        dropdown.canSelect = function(value) return GP.UI.RosterColumns:CanSelect(index, value) end
        dropdown:SetPoint("LEFT", label, "RIGHT", 10, 0)
        dropdown:SetValue(GP:IsForeverClient() and GP.UI.RosterColumns:Source(index, true) or columnSettings.columns[index])
        dropdowns[index] = dropdown
        dropdown:HookScript("OnClick", function()
            for other, button in ipairs(dropdowns) do
                if other ~= index then button.panel:Hide() end
            end
        end)
        previous = label
    end
    local noteColumnDesc = addOptionDescription(rosterDisplay, previous,
        L["Choose the last four roster columns, from left to right. Officer-only notes fall back to public notes without permission. Location is shown for online members; scores and points use the latest available roster data."], 380)
    noteColumnDesc:ClearAllPoints()
    noteColumnDesc:SetPoint("TOPLEFT", previous, "BOTTOMLEFT", 0, -20)

    local scanLoginCheck = createCheck(rosterDisplay, L["Scan after login"], function(value)
        scanSettings.login = value
    end)
    scanLoginCheck:SetPoint("TOPLEFT", noteColumnDesc, "BOTTOMLEFT", -26, -10)
...
local function companionModule()
    return GP:GetModule("Companion", true)
end

local function companionStatusText(info)
    local L = GP.L
    if not info or not info.hasStatus then
        return L["Companion data has not loaded. Run Guild Paragon Companion, restart WoW after first setup, and enable Guild Paragon Companion Data."]
    end
    if not info.installed then
        if info.setupComplete then
            return L["Companion is set up for this guild. This computer has no configured Companion data, so settings can be viewed but fresh data cannot be applied here."]
        end
        return L["Configure Guild Paragon Companion and run a sync, then reload."]
    end
    if not info.hasRoster then
        return L["Companion status loaded, but roster data did not load. Run the companion sync, then reload."]
    end
    if not info.contextMatches then
        return string.format(
            L["Companion data is for %s-%s, not the current guild. Run the companion sync for this guild, then reload."],
            info.guild ~= "" and info.guild or L["unknown"],
            info.realm ~= "" and info.realm or L["unknown"]
        )
    end
    return string.format(
        L["Companion data ready: %d member(s), status %s, generated %s."],
        tonumber(info.memberCount or info.lastRosterCount) or 0,
        info.statusText ~= "" and info.statusText or L["unknown"],
        formatTimestamp(info.generatedAt or info.lastSyncAt)
    )
end

local function refreshCompanionControls(frame)
    local Companion = companionModule()
    if not frame or not Companion then return end
    local L = GP.L
    local info = Companion:GetDisplayStatus()
    local canView = info.canView
    local setupComplete = info.setupComplete
    local canStartSetup = canView and info.contextMatches and not setupComplete

    if frame.companionSetupSection then
        frame.companionSetupSection:SetShown(canStartSetup)
    end
    if frame.companionSetupText then
        frame.companionSetupText:SetText(companionStatusText(info))
    end
    if frame.companionSetupButton then
        frame.companionSetupButton:SetEnabled(canStartSetup)
    end
    if frame.companionTabButton then
        frame.companionTabButton:SetShown(canView and setupComplete)
    end
    if frame.companionStatusText then
        frame.companionStatusText:SetText(companionStatusText(info))
    end
    if frame.companionModeText then
        local modeText = info.canEdit and L["Guild master edit mode"] or L["Officer read-only mode"]
        frame.companionModeText:SetText(modeText)
    end
    if frame.companionSavedText then
        local config = Companion:GetSavedConfig()
        if config then
            frame.companionSavedText:SetText(string.format(
                L["Saved setup: %s, %d field(s), auto-apply %s, updated %s by %s."],
                config.destination == "officer" and L["Custom Officer Notes"] or L["Custom Notes"],
                #(config.fields or {}),
                config.autoApply and L["on"] or L["off"],
                formatTimestamp(config.updatedAt or config.setupAt),
                config.updatedBy ~= "" and config.updatedBy or (config.setupBy ~= "" and config.setupBy or L["unknown"])
            ))
        else
            frame.companionSavedText:SetText(L["Setup has not been saved for this guild yet."])
        end
    end
end

local function ensureCompanionDraft(frame)
    if frame.companionDraft then return frame.companionDraft end
    local Companion = companionModule()
    local orderedFields = {}
    local selected = { spec = true, itemLevelEquipped = true }
    local saved = Companion and Companion:GetSavedConfig()
    if saved and #(saved.fields or {}) > 0 then
        selected = {}
        for _, key in ipairs(saved.fields or {}) do
            orderedFields[#orderedFields + 1] = key
            selected[key] = true
        end
    end
    local seen = {}
    for _, key in ipairs(orderedFields) do
        seen[key] = true
    end
    for _, field in ipairs(Companion and Companion:GetFieldDefinitions() or {}) do
        if not seen[field.key] then
            orderedFields[#orderedFields + 1] = field.key
        end
    end
    frame.companionDraft = {
        destination = saved and saved.destination or "general",
        orderedFields = orderedFields,
        selected = selected,
        autoApply = saved and saved.autoApply and true or false,
        conflictAction = "move",
    }
    return frame.companionDraft
end

local function selectedCompanionFields(draft)
    local fields = {}
    for _, key in ipairs(draft.orderedFields or {}) do
        if draft.selected and draft.selected[key] then fields[#fields + 1] = key end
    end
    return fields
end

local refreshCompanionSetupDraft
local renderCompanionPreview
local openCompanionPreviewDialog

local function companionPreviewLabel(row)
    local L = GP.L
    if row and row.matchType == "missing" then return L["Missing"] end
    return row and row.changed and L["Change"] or L["Same"]
end

local function companionTruncate(text, maxChars)
    if GP.CanAccessValue and not GP:CanAccessValue(text) then return "" end
    text = GP.SafeString and GP:SafeString(text, "") or text
    text = tostring(text or "")
    if #text > maxChars then
        return text:sub(1, math.max(0, maxChars - 3)) .. "..."
    end
    return text
end

local function createCompanionPreviewRow(parent)
    local row = CreateFrame("Frame", nil, parent)

    row.status = row:CreateFontString(nil, "ARTWORK")
    row.status:SetFontObject(Theme.font.small)
    row.status:SetPoint("LEFT", 0, 0)
    row.status:SetWidth(64)
    row.status:SetJustifyH("LEFT")

    row.name = row:CreateFontString(nil, "ARTWORK")
    row.name:SetFontObject(Theme.font.small)
    row.name:SetPoint("LEFT", row.status, "RIGHT", 8, 0)
    row.name:SetWidth(170)
    row.name:SetJustifyH("LEFT")

    row.current = row:CreateFontString(nil, "ARTWORK")
    row.current:SetFontObject(Theme.font.small)
    row.current:SetPoint("LEFT", row.name, "RIGHT", 10, 0)
    row.current:SetWidth(210)
    row.current:SetJustifyH("LEFT")

    row.desired = row:CreateFontString(nil, "ARTWORK")
    row.desired:SetFontObject(Theme.font.small)
    row.desired:SetPoint("LEFT", row.current, "RIGHT", 10, 0)
    row.desired:SetPoint("RIGHT", 0, 0)
    row.desired:SetJustifyH("LEFT")

    return row
end

local function updateCompanionPreviewRow(row, item)
    local changed = item and item.changed
    local missing = item and item.matchType == "missing"
    row.status:SetText(companionPreviewLabel(item))
    row.name:SetText(companionTruncate(item and item.name or "?", 24))
    row.current:SetText(companionTruncate(item and item.current or "", 34))
    row.desired:SetText(companionTruncate(item and item.desired or "", 46))
    if missing then
        row.status:SetTextColor(unpack(Theme.color.warning))
    elseif changed then
        row.status:SetTextColor(unpack(Theme.color.info))
    else
        row.status:SetTextColor(unpack(Theme.color.textSecondary))
    end
    row.name:SetTextColor(unpack(Theme.color.textPrimary))
    row.current:SetTextColor(unpack(Theme.color.textSecondary))
    row.desired:SetTextColor(unpack(changed and Theme.color.textPrimary or Theme.color.textSecondary))
end

local function ensureCompanionPreviewDialog(frame)
    if frame.companionPreviewDialog then return frame.companionPreviewDialog end

    local L = GP.L
    local dialog = Theme:CreatePanel(UIParent, "panelRaised", "accent")
    dialog:SetSize(790, 500)
    ...
    title:SetText(L["Companion Preview"])

    local close = Theme:CreateCloseButton(dialog)
    close:SetPoint("TOPRIGHT", -8, -8)
    close:SetScript("OnClick", function() dialog:Hide() end)

    local summary = dialog:CreateFontString(nil, "ARTWORK")
    summary:SetFontObject(Theme.font.small)
    summary:SetTextColor(unpack(Theme.color.textSecondary))
    ...
    header:SetHeight(20)
    header:SetPoint("TOPLEFT", summary, "BOTTOMLEFT", 0, -14)
    header:SetPoint("RIGHT", -16, 0)

    local function addHeader(text, point, width)
        local label = header:CreateFontString(nil, "ARTWORK")
        label:SetFontObject(Theme.font.small)
        label:SetTextColor(unpack(Theme.color.textSecondary))
        label:SetJustifyH("LEFT")
        label:SetText(text)
        if point then
            label:SetPoint("LEFT", point, "RIGHT", 10, 0)
        else
            label:SetPoint("LEFT")
        end
    ...
    local statusHeader = addHeader(L["Status"], nil, 64)
    local nameHeader = addHeader(L["Name"], statusHeader, 170)
    local currentHeader = addHeader(L["Current"], nameHeader, 210)
    addHeader(L["Companion"], currentHeader, 270)

    local listPanel = Theme:CreatePanel(dialog, "panel", "border")
    listPanel:SetPoint("TOPLEFT", header, "BOTTOMLEFT", 0, -4)
    listPanel:SetPoint("BOTTOMRIGHT", -16, 48)

    dialog.list = GP.UI.ScrollList:New(listPanel, COMPANION_PREVIEW_ROW_HEIGHT, createCompanionPreviewRow)
    dialog.list:SetUpdateRow(updateCompanionPreviewRow)

    local closeButton = Theme:CreateButton(dialog, L["Close"])
    closeButton:SetPoint("BOTTOMRIGHT", -16, 14)
    closeButton:SetScript("OnClick", function() dialog:Hide() end)

    frame.companionPreviewDialog = dialog
    return dialog
end

local function companionCanConfigure(frame)
    local Companion = companionModule()
    if not Companion then return false end
    if Companion:IsSetupComplete() then return Companion:CanEdit() end
    local info = Companion:GetDisplayStatus()
    return info.canView and info.contextMatches
end

local function refreshCompanionActionButtons(frame)
    local Companion = companionModule()
    if not frame or not Companion then return end
    local L = GP.L
    local draft = ensureCompanionDraft(frame)
    local fields = selectedCompanionFields(draft)
    local canConfigure = companionCanConfigure(frame)
    local setupComplete = Companion:IsSetupComplete()
    local info = Companion:GetDisplayStatus()
    local applyState = Companion:GetApplyStatus()
    local migrationState = Companion:GetMigrationStatus()
    local autoState = Companion.GetAutoApplyStatus and Companion:GetAutoApplyStatus() or nil
    local applying = applyState and applyState.running
    local migrating = migrationState and migrationState.running
    local changed = frame.companionLastPreviewSummary and (tonumber(frame.companionLastPreviewSummary.changed) or 0) or 0
    local conflicts = frame.companionConflictSummary and (tonumber(frame.companionConflictSummary.total) or 0) or 0

    if frame.companionSaveButton then
        frame.companionSaveButton:SetShown(canConfigure)
        frame.companionSaveButton:SetEnabled(canConfigure and #fields > 0 and not applying and not migrating)
        frame.companionSaveButton.text:SetText(setupComplete and L["Save Companion Settings"] or L["Save Companion Setup"])
    end
    if frame.companionPrepareButton then
        frame.companionPrepareButton:SetShown(setupComplete and Companion:CanEdit())
        frame.companionPrepareButton:SetEnabled(setupComplete and Companion:CanEdit() and info.contextMatches and conflicts > 0 and not applying and not migrating)
    end
    if frame.companionApplyButton then
        frame.companionApplyButton:SetShown(setupComplete and Companion:CanEdit())
        frame.companionApplyButton:SetEnabled(setupComplete and Companion:CanEdit() and info.contextMatches and changed > 0 and not applying and not migrating)
    end
    if frame.companionDisableButton then
        frame.companionDisableButton:SetShown(setupComplete and Companion:CanEdit())
        frame.companionDisableButton:SetEnabled(Companion:CanEdit() and not applying and not migrating)
    end
    if frame.companionApplyStatusText then
        if migrating then
            frame.companionApplyStatusText:SetText(string.format(
                L["Preparing existing notes: %d/%d complete."],
                tonumber(migrationState.moved or 0) + tonumber(migrationState.dropped or 0) + tonumber(migrationState.failed or 0),
                tonumber(migrationState.total or 0)
            ))
        elseif applying then
            frame.companionApplyStatusText:SetText(string.format(
                L["Applying Companion notes: %d/%d complete."],
                tonumber(applyState.applied or 0) + tonumber(applyState.failed or 0),
                tonumber(applyState.total or 0)
            ))
        elseif migrationState and migrationState.finishedAt then
            frame.companionApplyStatusText:SetText(string.format(
                L["Existing note preparation finished: %d moved, %d cleared, %d dropped, %d failed."],
                tonumber(migrationState.moved or 0),
                tonumber(migrationState.cleared or 0),
                tonumber(migrationState.dropped or 0),
                tonumber(migrationState.failed or 0)
            ))
        elseif applyState and applyState.finishedAt then
            frame.companionApplyStatusText:SetText(string.format(
                L["Companion note apply finished: %d applied, %d failed."],
                tonumber(applyState.applied or 0),
                tonumber(applyState.failed or 0)
            ))
        elseif autoState and autoState.pending then
            frame.companionApplyStatusText:SetText(L["Companion auto-apply is waiting for startup sync to finish."])
        elseif autoState and autoState.finishedAt and tonumber(autoState.applied or 0) == 0 then
            frame.companionApplyStatusText:SetText(L["Companion auto-apply checked: no note changes needed."])
        elseif autoState and autoState.skippedAt and autoState.skippedReason then
            frame.companionApplyStatusText:SetText(autoState.skippedReason)
        else
            frame.companionApplyStatusText:SetText("")
        end
    end
    if frame.companionLockText then
        if canConfigure then
            frame.companionLockText:SetText(setupComplete
                and L["Apply Companion Notes writes only previewed differences in small batches."]
                or L["Saving setup only stores Companion settings. It does not write guild notes yet."])
        elseif setupComplete then
            frame.companionLockText:SetText(L["Companion settings are read-only for officers after setup."])
        else
            frame.companionLockText:SetText(L["Companion setup requires matching local Companion data."])
        end
    end
end

local function refreshCompanionConflictPreview(frame)
    local L = GP.L
    local Companion = companionModule()
    if not frame or not Companion then return end
    local draft = ensureCompanionDraft(frame)
    local action = draft.conflictAction == "clear" and "clear" or "move"
    local showConflicts = Companion:IsSetupComplete() and Companion:CanEdit()

    if frame.companionConflictLabel then frame.companionConflictLabel:SetShown(showConflicts) end
    if frame.companionConflictSummaryText then frame.companionConflictSummaryText:SetShown(showConflicts) end
    if frame.companionConflictRows then frame.companionConflictRows:SetShown(showConflicts) end

    if frame.companionConflictButtons then
        for key, button in pairs(frame.companionConflictButtons) do
            button:SetShown(showConflicts)
            button:SetSelected(action == key)
            button:SetEnabled(showConflicts and not Companion:IsApplyRunning() and not Companion:IsMigrationRunning())
        end
    end

    if not showConflicts then
        frame.companionConflictSummary = nil
        refreshCompanionActionButtons(frame)
        return
    end

    local summary, rows, err = Companion:BuildManagedNoteConflicts(action)
    frame.companionConflictSummary = summary

    if frame.companionConflictSummaryText then
        if err then
            frame.companionConflictSummaryText:SetText(err)
        elseif action == "move" then
            frame.companionConflictSummaryText:SetText(string.format(
                L["Existing managed-field notes: %d found, %d can move, %d would be dropped."],
                summary.total or 0,
                summary.moved or 0,
                summary.dropped or 0
            ))
        else
            frame.companionConflictSummaryText:SetText(string.format(
                L["Existing managed-field notes: %d found, %d would be cleared."],
                summary.total or 0,
                summary.cleared or 0
            ))
        end
    end

    local lines = {}
    local limit = math.min(#(rows or {}), 1)
    for index = 1, limit do
        local row = rows[index]
        local prefix = row.move and L["Move"] or L["Clear"]
        if row.drop then prefix = L["Drop"] end
        lines[#lines + 1] = string.format("%s: %s -> %s", prefix, row.name or "?", row.source or "")
    end
    if rows and #rows > limit then
        lines[#lines + 1] = string.format(L["%d more conflict row(s) not shown."], #rows - limit)
    end
    if #lines == 0 and not err then
        lines[#lines + 1] = L["No existing managed-field notes need preparation."]
    end
    if frame.companionConflictRows then
        frame.companionConflictRows:SetText(table.concat(lines, "\n"))
    end
    refreshCompanionActionButtons(frame)
end

local function prepareCompanionExistingNotes(frame)
    local L = GP.L
    local Companion = companionModule()
    if not Companion then return end
    local draft = ensureCompanionDraft(frame)
    local ok, err = Companion:PrepareExistingNotes(draft.conflictAction)
    if ok then
        refreshCompanionActionButtons(frame)
        local state = Companion:GetMigrationStatus()
        if state and tonumber(state.total or 0) == 0 then
            setSettingsStatus(frame, L["No existing managed-field notes need preparation."])
        else
            setSettingsStatus(frame, L["Existing note preparation started."])
        end
    else
        setSettingsStatus(frame, err or L["Existing notes could not be prepared."], true)
    end
end

local function applyCompanionNotes(frame)
    local L = GP.L
    local Companion = companionModule()
    if not Companion then return end
    local ok, err = Companion:ApplySavedNotes()
    if ok then
        refreshCompanionActionButtons(frame)
        local state = Companion:GetApplyStatus()
        if state and tonumber(state.total or 0) == 0 then
            setSettingsStatus(frame, L["Companion notes already match the saved setup."])
        else
            setSettingsStatus(frame, L["Companion note apply started."])
        end
    else
        setSettingsStatus(frame, err or L["Companion notes could not be applied."], true)
    end
end

local function saveCompanionSettings(frame)
    local L = GP.L
    local Companion = companionModule()
    if not Companion then return end
    local draft = ensureCompanionDraft(frame)
    local ok, err = Companion:SaveSettings({
        destination = draft.destination,
        fields = selectedCompanionFields(draft),
        autoApply = draft.autoApply,
    })
    if ok then
        frame.companionSetupMode = false
        refreshCompanionControls(frame)
        refreshCompanionSetupDraft(frame)
        renderCompanionPreview(frame)
        Settings:SelectSettingsPage(frame, "companion")
        setSettingsStatus(frame, L["Companion setup saved. No guild notes were written."])
    else
        setSettingsStatus(frame, err or L["Companion setup could not be saved."], true)
    end
end

local function disableCompanion(frame)
    local L = GP.L
    local Companion = companionModule()
    if not Companion then return end
    local ok, err = Companion:Disable()
    if ok then
        frame.companionDraft = nil
        frame.companionSetupMode = false
        refreshCompanionControls(frame)
        refreshCompanionSetupDraft(frame)
        renderCompanionPreview(frame)
        Settings:SelectSettingsPage(frame, "general")
        setSettingsStatus(frame, L["Companion disabled for this guild."])
    else
        setSettingsStatus(frame, err or L["Companion could not be disabled."], true)
    end
end

refreshCompanionSetupDraft = function(frame)
    local L = GP.L
    local Companion = companionModule()
    if not frame or not Companion then return end
    local draft = ensureCompanionDraft(frame)
    ...
    if frame.companionDestinationButtons then
        for key, button in pairs(frame.companionDestinationButtons) do
            button:SetSelected(draft.destination == key)
            button:SetEnabled(canConfigure)
        end
    end
    if frame.companionAutoApplyCheck then
        frame.companionAutoApplyCheck:SetChecked(draft.autoApply and true or false)
        frame.companionAutoApplyCheck:SetEnabled(canConfigure)
        frame.companionAutoApplyCheck:SetAlpha(canConfigure and 1 or 0.55)
    end

    local fieldsByKey = {}
    for _, field in ipairs(Companion:GetFieldDefinitions()) do
        fieldsByKey[field.key] = field
    end
    for index, row in ipairs(frame.companionFieldRows or {}) do
        local key = draft.orderedFields[index]
        local field = fieldsByKey[key]
        row.index = index
        row.key = key
        row.check:SetChecked(draft.selected[key] and true or false)
        row.check:SetEnabled(canConfigure)
        row.label:SetText(field and L[field.labelKey] or key)
        row.up:SetEnabled(canConfigure and index > 1)
        row.down:SetEnabled(canConfigure and index < #draft.orderedFields)
    end
    refreshCompanionActionButtons(frame)
end

renderCompanionPreview = function(frame)
    if not frame then return end
    local page = frame.settingPages and frame.settingPages.companion
    if not page or not page:IsVisible() then
        frame.companionPreviewDirty = true
        return
    end
    frame.companionPreviewDirty = false
    local L = GP.L
    local Companion = companionModule()
    if not frame or not Companion then return end
    local draft = ensureCompanionDraft(frame)
    local fields = selectedCompanionFields(draft)
    local summary, rows = GP:MonitorCall("ui.companionPreview", Companion.BuildPreview, Companion, { destination = draft.destination, fields = fields })
    frame.companionLastPreviewSummary = summary
    frame.companionLastPreviewRows = rows

    if frame.companionPreviewSummary then
        if summary.blocked then
            frame.companionPreviewSummary:SetText(summary.blockedReason or L["Companion note checks are paused while combat or protected content is active."])
        elseif #fields == 0 then
            frame.companionPreviewSummary:SetText(L["Select at least one field to preview Companion notes."])
        else
            frame.companionPreviewSummary:SetText(string.format(
                L["Preview: %d matched, %d would change, %d already match, %d missing, %d blank."],
                summary.matched or 0,
                summary.changed or 0,
                summary.unchanged or 0,
    ...
    if frame.companionPreviewRows then
        if #rows > 0 then
            frame.companionPreviewRows:SetText(L["Open Preview to review every Companion row before applying changes."])
        elseif #fields > 0 then
            frame.companionPreviewRows:SetText(L["No preview rows available yet."])
        else
            frame.companionPreviewRows:SetText("")
        end
    end
    refreshCompanionConflictPreview(frame)
    refreshCompanionActionButtons(frame)
end

openCompanionPreviewDialog = function(frame)
    local L = GP.L
    if not frame then return end
    renderCompanionPreview(frame)

    local summary = frame.companionLastPreviewSummary or {}
    local rows = frame.companionLastPreviewRows or {}
    local dialog = ensureCompanionPreviewDialog(frame)
    dialog.summary:SetText(string.format(
        L["Preview: %d matched, %d would change, %d already match, %d missing, %d blank."],
        summary.matched or 0,
        summary.changed or 0,
        summary.unchanged or 0,
    ...
    dialog.list:SetData(rows, true)
    dialog:Show()
    dialog:Raise()
end

local function buildCompanionSetupSection(frame, generalPage, anchor)
    local L = GP.L
    local section = createSection(generalPage, L["Companion"])
    section:SetPoint("TOPLEFT", anchor, "BOTTOMLEFT", 0, -SECTION_GAP)
    section:SetSize(SETTINGS_COLUMN_WIDTH, 130)
    section:Hide()
    frame.companionSetupSection = section

    local text = addBodyText(section, nil, "", SETTINGS_COLUMN_WIDTH - Theme.layout.gutter * 2)
    text:SetFontObject(Theme.font.small)
    text:SetTextColor(unpack(Theme.color.textSecondary))
    frame.companionSetupText = text

    local button = Theme:CreateButton(section, L["Set Up Companion"])
    button:SetPoint("TOPLEFT", text, "BOTTOMLEFT", 0, -14)
    button:SetScript("OnClick", function()
        frame.companionSetupMode = true
        Settings:SelectSettingsPage(frame, "companion")
        setSettingsStatus(frame, L["Companion setup preview opened."])
    end)
    frame.companionSetupButton = button
end

local function buildCompanionPage(frame, companionPage)
    local L = GP.L
    local section = createSection(companionPage, L["Companion"])
    section:SetPoint("TOPLEFT", 0, 0)
    section:SetSize(COMPANION_PAGE_WIDTH, COMPANION_PAGE_HEIGHT)

    local companionTextWidth = COMPANION_PAGE_WIDTH - Theme.layout.gutter * 2

    local statusText = addBodyText(section, nil, "", companionTextWidth)
    statusText:SetFontObject(Theme.font.body)
    frame.companionStatusText = statusText

    local modeText = addBodyText(section, statusText, "", companionTextWidth)
    modeText:SetFontObject(Theme.font.small)
    modeText:SetTextColor(unpack(Theme.color.textSecondary))
    frame.companionModeText = modeText

    local savedText = addBodyText(section, modeText, "", companionTextWidth)
    savedText:SetFontObject(Theme.font.small)
    savedText:SetTextColor(unpack(Theme.color.textSecondary))
    frame.companionSavedText = savedText

    local plannedText = addBodyText(section, savedText, L["Setup will choose Custom Notes or Custom Officer Notes, select ordered fields, and preview changes before writing."], companionTextWidth)
    plannedText:SetFontObject(Theme.font.small)
    plannedText:SetTextColor(unpack(Theme.color.textSecondary))

    local destinationLabel = section:CreateFontString(nil, "ARTWORK")
    destinationLabel:SetFontObject(Theme.font.heading)
    destinationLabel:SetPoint("TOPLEFT", plannedText, "BOTTOMLEFT", 0, -18)
    destinationLabel:SetText(L["Managed note field"])

    frame.companionDestinationButtons = {}
    local generalButton = createSettingsTabButton(section, L["Custom Notes"])
    generalButton:SetPoint("TOPLEFT", destinationLabel, "BOTTOMLEFT", 0, -8)
    generalButton:SetScript("OnClick", function()
        ensureCompanionDraft(frame).destination = "general"
        refreshCompanionSetupDraft(frame)
        renderCompanionPreview(frame)
    end)
    frame.companionDestinationButtons.general = generalButton

    local officerButton = createSettingsTabButton(section, L["Custom Officer Notes"])
    officerButton:SetPoint("LEFT", generalButton, "RIGHT", 8, 0)
    officerButton:SetScript("OnClick", function()
        ensureCompanionDraft(frame).destination = "officer"
        refreshCompanionSetupDraft(frame)
        renderCompanionPreview(frame)
    end)
    frame.companionDestinationButtons.officer = officerButton

    local autoApplyCheck = createCheck(section, L["Auto-apply Companion notes after login or reload"], function(value)
        ensureCompanionDraft(frame).autoApply = value and true or false
        refreshCompanionActionButtons(frame)
    end)
    autoApplyCheck:SetPoint("TOPLEFT", generalButton, "BOTTOMLEFT", -4, -10)
    frame.companionAutoApplyCheck = autoApplyCheck

    local autoApplyDesc = addBodyText(section, autoApplyCheck, L["Runs one startup check after sync is idle and writes only changed Companion-managed notes."], companionTextWidth)
    autoApplyDesc:SetFontObject(Theme.font.small)
    autoApplyDesc:SetTextColor(unpack(Theme.color.textSecondary))

    local fieldsLabel = section:CreateFontString(nil, "ARTWORK")
    fieldsLabel:SetFontObject(Theme.font.heading)
    fieldsLabel:SetPoint("TOPLEFT", autoApplyDesc, "BOTTOMLEFT", 0, -14)
    fieldsLabel:SetText(L["Note fields and order"])

    frame.companionFieldRows = {}
    for index = 1, 10 do
        local row = CreateFrame("Frame", nil, section)
        row:SetSize(420, 24)
        if index == 1 then
            row:SetPoint("TOPLEFT", fieldsLabel, "BOTTOMLEFT", 0, -8)
        elseif index == 6 then
            row:SetPoint("TOPLEFT", fieldsLabel, "BOTTOMLEFT", 460, -8)
        else
            row:SetPoint("TOPLEFT", frame.companionFieldRows[index - 1], "BOTTOMLEFT", 0, -6)
        end

        row.check = createCheck(row, "")
        row.check:SetPoint("LEFT", 0, 0)
        row.label = row.check.text
        row.check:SetScript("OnClick", function()
            local draft = ensureCompanionDraft(frame)
            draft.selected[row.key] = row.check:GetChecked() and true or nil
            refreshCompanionActionButtons(frame)
            renderCompanionPreview(frame)
        end)

        row.up = Theme:CreateButton(row, L["Up"])
        row.up:SetWidth(52)
        row.up:SetPoint("LEFT", row.check, "RIGHT", 210, 0)
        row.up:SetScript("OnClick", function()
            local draft = ensureCompanionDraft(frame)
            local rowIndex = row.index or 1
            if rowIndex <= 1 then return end
            draft.orderedFields[rowIndex], draft.orderedFields[rowIndex - 1] = draft.orderedFields[rowIndex - 1], draft.orderedFields[rowIndex]
            refreshCompanionSetupDraft(frame)
            renderCompanionPreview(frame)
        end)

        row.down = Theme:CreateButton(row, L["Down"])
        row.down:SetWidth(62)
        row.down:SetPoint("LEFT", row.up, "RIGHT", 6, 0)
        row.down:SetScript("OnClick", function()
            local draft = ensureCompanionDraft(frame)
            local rowIndex = row.index or #draft.orderedFields
            if rowIndex >= #draft.orderedFields then return end
            draft.orderedFields[rowIndex], draft.orderedFields[rowIndex + 1] = draft.orderedFields[rowIndex + 1], draft.orderedFields[rowIndex]
            refreshCompanionSetupDraft(frame)
            renderCompanionPreview(frame)
        end)

        frame.companionFieldRows[index] = row
    end

    local conflictLabel = section:CreateFontString(nil, "ARTWORK")
    conflictLabel:SetFontObject(Theme.font.heading)
    conflictLabel:SetPoint("TOPLEFT", frame.companionFieldRows[5], "BOTTOMLEFT", 0, -16)
    conflictLabel:SetText(L["Existing managed-field notes"])
    frame.companionConflictLabel = conflictLabel

    frame.companionConflictButtons = {}
    local moveButton = createSettingsTabButton(section, L["Move existing notes"])
    moveButton:SetPoint("TOPLEFT", conflictLabel, "BOTTOMLEFT", 0, -8)
    moveButton:SetScript("OnClick", function()
        ensureCompanionDraft(frame).conflictAction = "move"
        refreshCompanionConflictPreview(frame)
    end)
    frame.companionConflictButtons.move = moveButton

    local clearButton = createSettingsTabButton(section, L["Clear existing notes"])
    clearButton:SetPoint("LEFT", moveButton, "RIGHT", 8, 0)
    clearButton:SetScript("OnClick", function()
        ensureCompanionDraft(frame).conflictAction = "clear"
        refreshCompanionConflictPreview(frame)
    end)
    frame.companionConflictButtons.clear = clearButton

    local conflictSummary = addBodyText(section, moveButton, "", companionTextWidth)
    conflictSummary:SetFontObject(Theme.font.small)
    conflictSummary:SetTextColor(unpack(Theme.color.textSecondary))
    frame.companionConflictSummaryText = conflictSummary

    local conflictRows = addBodyText(section, conflictSummary, "", companionTextWidth)
    conflictRows:SetFontObject(Theme.font.small)
    conflictRows:SetTextColor(unpack(Theme.color.textSecondary))
    conflictRows:SetSpacing(2)
    frame.companionConflictRows = conflictRows

    local previewButton = Theme:CreateButton(section, L["Preview"])
    previewButton:SetPoint("TOPLEFT", conflictRows, "BOTTOMLEFT", 0, -14)
    previewButton:SetScript("OnClick", function() openCompanionPreviewDialog(frame) end)

    local saveButton = Theme:CreateButton(section, L["Save Companion Setup"])
    saveButton:SetPoint("LEFT", previewButton, "RIGHT", 8, 0)
    saveButton:SetWidth(170)
    saveButton:SetScript("OnClick", function() saveCompanionSettings(frame) end)
    frame.companionSaveButton = saveButton

    local prepareButton = Theme:CreateButton(section, L["Prepare Existing Notes"])
    prepareButton:SetPoint("LEFT", saveButton, "RIGHT", 8, 0)
    prepareButton:SetWidth(180)
    prepareButton:SetScript("OnClick", function() prepareCompanionExistingNotes(frame) end)
    frame.companionPrepareButton = prepareButton

    local applyButton = Theme:CreateButton(section, L["Apply Companion Notes"])
    applyButton:SetPoint("LEFT", prepareButton, "RIGHT", 8, 0)
    applyButton:SetWidth(170)
    applyButton:SetScript("OnClick", function() applyCompanionNotes(frame) end)
    frame.companionApplyButton = applyButton

    local disableButton = Theme:CreateButton(section, L["Disable Companion"])
    disableButton:SetPoint("LEFT", applyButton, "RIGHT", 8, 0)
    disableButton:SetWidth(150)
    disableButton:SetScript("OnClick", function() disableCompanion(frame) end)
    frame.companionDisableButton = disableButton

    local lockText = addBodyText(section, previewButton, "", companionTextWidth)
    lockText:SetFontObject(Theme.font.small)
    lockText:SetTextColor(unpack(Theme.color.textSecondary))
    frame.companionLockText = lockText

    local applyStatusText = addBodyText(section, lockText, "", companionTextWidth)
    applyStatusText:SetFontObject(Theme.font.small)
    applyStatusText:SetTextColor(unpack(Theme.color.textSecondary))
    frame.companionApplyStatusText = applyStatusText

    local previewSummary = addBodyText(section, applyStatusText, L["Select fields, then preview the Companion-generated notes before setup writes anything."], companionTextWidth)
    previewSummary:SetFontObject(Theme.font.small)
    previewSummary:SetTextColor(unpack(Theme.color.textSecondary))
    frame.companionPreviewSummary = previewSummary

    local previewRows = addBodyText(section, previewSummary, "", companionTextWidth)
    previewRows:SetFontObject(Theme.font.small)
    previewRows:SetTextColor(unpack(Theme.color.textSecondary))
    previewRows:SetSpacing(2)
    frame.companionPreviewRows = previewRows
...
function Settings:Build(parent)
    ...
    frame.companionTabButton = addSettingsTab("companion", L["Companion"])
    ...
    local companionPage = createPage("companion")

    GP.UI.BackupRestoreTab:Build(backupPage)
    buildCompanionPage(frame, companionPage)
    ...
    Settings:RegisterMessage("GuildParagon_RosterScanned", GP:MonitorWrap("listener.settings", debouncedRefreshHealthMacroIgnores))
    ...
    if GP:HasHousingService() then
        local housingIconCheck = createCheck(leftTop, L["Show housing icon in roster table"], function(value)
            rosterSettings.showHousingIcon = value
            GP:SendMessage("GuildParagon_RosterDisplaySettingsChanged")
        end)
        housingIconCheck:SetPoint("TOPLEFT", minimapDesc, "BOTTOMLEFT", -26, -10)
        housingIconCheck:SetChecked(rosterSettings.showHousingIcon)
        addOptionDescription(leftTop, housingIconCheck, L["Shows a small housing marker beside roster members with a known subdivision. The detail panel still shows the full subdivision text."], 380)
    end

    GP.UI.BuildUIKeybindingSettings(leftTop)

    local safety = createSection(generalPage, L["Safety & Performance"])
    safety:SetPoint("TOPLEFT", leftTop, "BOTTOMLEFT", 0, -SECTION_GAP)
    safety:SetSize(SETTINGS_COLUMN_WIDTH, 265)
    ...
    GP.UI.RecruitmentExpiry:BuildSettings(safety, performanceDesc)
    GP.UI.BuildDateFormatSettings(safety, frame)

    buildCompanionSetupSection(frame, generalPage, safety)

    buildRosterDisplaySection(frame, generalPage, leftTop, rosterSettings)
    ...
    local lockedZonesDesc = addBodyText(lockedZones, lockedZones.heading,
        L[GP:IsForeverClient() and "Forever dungeon, raid and battleground defaults. The recruitment scanner uses this list when reviewing candidates."
            or "Built automatically from this season's Mythic dungeons/raids, common PvP battlegrounds and arenas, and Delves. The recruitment scanner uses this list when reviewing candidates."],
    ...
    Settings:RegisterMessage("GuildParagon_CompanionStatusChanged", function()
        if frame then refreshCompanionControls(frame) end
    end)
    Settings:RegisterMessage("GuildParagon_CompanionSettingsChanged", function()
        if not frame then return end
        frame.companionDraft = nil
        refreshCompanionControls(frame)
        refreshCompanionSetupDraft(frame)
    ...
    Settings:RegisterMessage("GuildParagon_CompanionApplyChanged", function()
        if not frame then return end
        refreshCompanionActionButtons(frame)
        local Companion = companionModule()
        if not Companion or not Companion:IsApplyRunning() then
            renderCompanionPreview(frame)
        end
    end)
    Settings:RegisterMessage("GuildParagon_CompanionMigrationChanged", function()
        if not frame then return end
        refreshCompanionActionButtons(frame)
        local Companion = companionModule()
        if not Companion or not Companion:IsMigrationRunning() then
            renderCompanionPreview(frame)
        end
    end)
    ...
    frame.OnSelected = function()
        frame.backupTabButton:SetShown(GP:IsOfficer())
        refreshCompanionControls(frame)
        if frame.companionPreviewDirty then renderCompanionPreview(frame) end
...
function Settings:SelectSettingsPage(frame, key)
    ...
    local Companion = companionModule()
    if key == "companion" and (not Companion or not Companion:CanView() or (not Companion:IsSetupComplete() and not frame.companionSetupMode)) then key = "general" end
...
function Settings:RefreshStatus(frame)
    ...
    local lastScan = guildData.lastScan and GP:DisplayDate("%Y-%m-%d %H:%M", guildData.lastScan) or L["never"]
