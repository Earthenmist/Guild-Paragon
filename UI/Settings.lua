...
local function companionStatusText(info)
    ...
    if not info or not info.hasStatus then
        return L["Companion files have not loaded. Install or configure Guild Paragon Companion, then reload."]
    end
    if not info.installed then
        if info.setupComplete then
            return L["Companion is set up for this guild. This computer has only blank Companion stubs, so settings can be viewed but fresh Companion data cannot be applied here."]
        end
        return L["Companion load stubs are present. Configure Guild Paragon Companion, then reload."]
    end
