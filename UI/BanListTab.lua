...
local function formatDate(ts)
    return type(ts) == "number" and GP:DisplayDate("%Y-%m-%d", ts) or ""
end
...
local function loadForm(record)
    ...
    frame.dateBox:SetText(type(record.bannedAt) == "number" and date("%Y-%m-%d", record.bannedAt) or "")
end
...
function BanListTab:Build(parent)
    ...
    BanListTab:RegisterMessage("GuildParagon_RosterScanned", GP:MonitorWrap("listener.bans", debouncedRefresh))
    self:Refresh()
    return frame
end
