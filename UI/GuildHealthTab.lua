...
local PAGE_IDS = { "overview", "info", "attention", "recruitment", "housing", "upcoming", "exports" }

local function pageAccessSignature(GuildHealth)
    local access = {}
    for _, pageID in ipairs(PAGE_IDS) do
        access[#access + 1] = GuildHealth:CanUsePage(pageID) and "1" or "0"
    end
    return table.concat(access)
end
...
local function formatDateTime(ts)
    return type(ts) == "number" and GP:DisplayDate("%Y-%m-%d %H:%M", ts) or GP.L["Never"]
end
...
local function formatShortTime(ts)
    return type(ts) == "number" and GP:DisplayDate("%m-%d %H:%M", ts) or GP.L["Never"]
end
...
local function selectView(key)
    ...
    if not (frame.viewPages and frame.viewPages[key]) then
        key = frame.viewButtons and frame.viewButtons[1] and frame.viewButtons[1].key or nil
    end
    if not key or not frame.viewPages[key] then return end
    local previousView = activeView
    local previousPage = previousView and frame.viewPages and frame.viewPages[previousView]
    if previousView ~= key and previousPage and previousPage.OnDeselected then
        previousPage:OnDeselected()
    end
    activeView = key
...
local function buildVisibleRecruitmentExport()
    ...
    for _, record in ipairs(visibleRecruitmentRecords) do
        table.insert(lines, tsvLine({
            type(record.endedAt) == "number" and date("%Y-%m-%d %H:%M", record.endedAt) or GP.L["Never"],
            record.type,
...
local function updateOverview(summary)
    ...
    if GP:IsForeverClient() then
        mythic = overview.levelDistribution or {}
        frame.overview.mythicSummary:SetText(string.format(GP.L["%d members counted / %d total; %d unknown level"],
            mythic.known or 0, mythic.total or 0, mythic.unknown or 0))
    else
...
local function applyRefreshResults(summary, analyticsSummary)
    if not frame or not summary then return end
    analyticsSummary = analyticsSummary or {}
    frame.cards.extra:Show()
    local totals, today = analyticsSummary.totals or {}, analyticsSummary.today or {}
    if frame.recruitmentTodayText then frame.recruitmentTodayText:SetText(buildMetricBlock(today)) end
    if frame.recruitmentLifetimeText then frame.recruitmentLifetimeText:SetText(buildMetricBlock(totals)) end
    if activeView == "recruitment" then
        ...
        if GP:IsForeverClient() then
            local dates = GP:GetModule("UpcomingDates")
            local settings = dates:GetSettings()
            local count = 0
            if settings.showBirthdays then
                for _, row in ipairs(dates:GetRows()) do
                    if row.type == "birthday" then count = count + 1 end
                end
            end
            setCardTitle(frame.cards.extra, GP.L["Upcoming Birthdays"])
            setCard(frame.cards.extra, settings.showBirthdays and tostring(count) or "-",
                settings.showBirthdays and string.format(GP.L["Next %d days"], settings.reminderWindowDays)
                    or GP.L["Birthday reminders hidden"],
                count > 0 and Theme.color.info or Theme.color.textPrimary)
            setCardAction(frame.cards.extra, function() selectView("upcoming") end)
        end
    end
    ...
    if retentionList and recruitmentActivityList then
        ...
        wipe(visibleRecruiters)
        wipe(visibleRecruitmentRecords)
    end
end

function GuildHealthTab:Refresh()
    if not frame then return end
    if frame.guildRecruitment then frame.guildRecruitment:SetShown(GP.UI.GuildRecruitment:IsOfficer()) end
    if activeView == "info" and frame.viewPages and frame.viewPages.info then frame.viewPages.info:Refresh() end
    ...
    if not GuildHealth:CanUsePage("overview") then return end
    ...
    GP:RunFrameSteps("GuildHealthTab:Refresh", {
        ...
        function()
            if token ~= refreshToken or not frame or not frame:IsShown() then return false end
            if GuildHealth:CanUsePage("recruitment") then
...
local function clearOfficerPageState()
    refreshToken = refreshToken + 1
    refreshPending = false
    selectedRow = nil
    wipe(visibleRows)
    ...
    attentionList = nil
    retentionList = nil
    recruitmentActivityList = nil
end

local function buildMemberShell(parent)
    local L = GP.L
    local GuildHealth = GP:GetModule("GuildHealth")
    local Recruitment = GP:GetModule("Recruitment")
    local member = { pages = {}, buttons = {}, active = nil }
    clearOfficerPageState()
    frame = CreateFrame("Frame", nil, parent)
    frame:SetAllPoints()
    frame.accessMode = "member"
    frame.accessSignature = pageAccessSignature(GuildHealth)

    local heading = addText(frame, Theme.font.title, L["Guild Health"], { "TOPLEFT" })
    local viewBar = CreateFrame("Frame", nil, frame)
    viewBar:SetPoint("TOPLEFT", heading, "BOTTOMLEFT", 0, -16)
    viewBar:SetPoint("RIGHT")
    viewBar:SetHeight(28)

    local content = CreateFrame("Frame", nil, frame)
    content:SetPoint("TOPLEFT", viewBar, "BOTTOMLEFT", 0, -Theme.layout.gutter)
    content:SetPoint("BOTTOMRIGHT")

    local pageRegistry = {
        {
            id = "info", label = L["Info"],
            build = function(page) GP.UI.GuildInfoPage:Build(page) end,
            refresh = function() if member.pages.info then member.pages.info:Refresh() end end,
        },
        {
            id = "recruitment",
            label = L["Recruitment"],
            build = function(page)
                local todayPanel = Theme:CreatePanel(page, "panel", "border")
                todayPanel:SetPoint("TOPLEFT")
                todayPanel:SetPoint("BOTTOMLEFT")
                todayPanel:SetWidth(360)
                local todayHeading = addText(todayPanel, Theme.font.heading, L["Today"], { "TOPLEFT", todayPanel, "TOPLEFT", Theme.layout.gutter, -Theme.layout.gutter })
                member.recruitmentToday = addText(todayPanel, Theme.font.body, "", { "TOPLEFT", todayHeading, "BOTTOMLEFT", 0, -10 })
                member.recruitmentToday:SetPoint("RIGHT", -Theme.layout.gutter, 0)
                member.recruitmentToday:SetJustifyV("TOP")

                local lifetimePanel = Theme:CreatePanel(page, "panel", "border")
                lifetimePanel:SetPoint("TOPLEFT", todayPanel, "TOPRIGHT", Theme.layout.gutter, 0)
                lifetimePanel:SetPoint("BOTTOMRIGHT")
                local lifetimeHeading = addText(lifetimePanel, Theme.font.heading, L["Lifetime"], { "TOPLEFT", lifetimePanel, "TOPLEFT", Theme.layout.gutter, -Theme.layout.gutter })
                member.recruitmentLifetime = addText(lifetimePanel, Theme.font.body, "", { "TOPLEFT", lifetimeHeading, "BOTTOMLEFT", 0, -10 })
                member.recruitmentLifetime:SetPoint("RIGHT", -Theme.layout.gutter, 0)
                member.recruitmentLifetime:SetJustifyV("TOP")
            end,
            refresh = function()
                if not GuildHealth:CanUsePage("recruitment") then return end
                local summary = Recruitment:GetAnalyticsSummary(nil, 100) or {}
                if member.recruitmentToday then member.recruitmentToday:SetText(buildMetricBlock(summary.today or {})) end
                if member.recruitmentLifetime then member.recruitmentLifetime:SetText(buildMetricBlock(summary.totals or {})) end
            end,
        },
        {
            id = "housing",
            label = L["Housing"],
            build = function(page)
                if GP.UI.HousingMap and GP.UI.HousingMap.Build then
                    member.housingContent = GP.UI.HousingMap:Build(page)
                end
            end,
            refresh = function()
                if member.housingContent and member.housingContent.OnSelected then
                    member.housingContent:OnSelected()
                elseif GP.UI.HousingMap then
                    GP.UI.HousingMap:Refresh()
                end
            end,
        },
        {
            id = "upcoming",
            label = L["Upcoming Dates"],
            build = function(page)
                if GP.UI.UpcomingDatesPage and GP.UI.UpcomingDatesPage.Build then
                    member.upcomingContent = GP.UI.UpcomingDatesPage:Build(page)
                end
            end,
            refresh = function()
                if member.upcomingContent and member.upcomingContent.Refresh then
                    member.upcomingContent:Refresh()
                end
            end,
        },
    }

    local function selectPage(pageID)
        if not GuildHealth:CanUsePage(pageID) then return false end
        local previousPage = member.active and member.pages[member.active]
        if member.active ~= pageID and previousPage and previousPage.OnDeselected then
            previousPage:OnDeselected()
        end
        for id, page in pairs(member.pages) do page:SetShown(id == pageID) end
        local record
        for _, candidate in ipairs(pageRegistry) do
            if candidate.id == pageID then record = candidate break end
        end
        if not record then return false end
        if not member.pages[pageID] then
            local page = CreateFrame("Frame", nil, content)
            page:SetAllPoints()
            member.pages[pageID] = page
            record.build(page)
        end
        member.pages[pageID]:Show()
        member.active = pageID
        for id, button in pairs(member.buttons) do
            setButtonSelected(button, id == pageID)
        end
        record.refresh()
        return true
    end
    ...
    local function refreshAccess()
        if GuildHealth:CanUsePage("overview") then
            GuildHealthTab:UnregisterAllMessages()
            if GP.UI.MainWindow then GP.UI.MainWindow:RebuildTab("guildhealth") end
            return
        end
        local previous
        local firstAllowed
        for _, record in ipairs(pageRegistry) do
            local allowed = GuildHealth:CanUsePage(record.id)
            local button = member.buttons[record.id]
            button:SetShown(allowed)
            button:ClearAllPoints()
            if allowed then
                firstAllowed = firstAllowed or record.id
                if previous then
                ...
                previous = button
            elseif member.pages[record.id] then
                member.pages[record.id]:Hide()
            end
        end
        if not member.active or not GuildHealth:CanUsePage(member.active) then
            member.active = nil
            if firstAllowed then selectPage(firstAllowed) end
        elseif frame:IsShown() then
            selectPage(member.active)
        end
        if not firstAllowed and GP.UI.MainWindow then
            GP.UI.MainWindow:BuildSidebar()
        end
    end

    for _, record in ipairs(pageRegistry) do
        local button = createFilterButton(viewBar, record.label, function() selectPage(record.id) end)
        button.key = record.id
        button:SetWidth(record.id == "info" and 60 or 100)
        member.buttons[record.id] = button
    end

    GuildHealthTab:RegisterMessage("GuildParagon_HousingChanged", function()
        if frame and frame:IsShown() and member.active == "housing" then
            selectPage("housing")
        end
    end)
    GuildHealthTab:RegisterMessage("GuildParagon_BirthdayChanged", function()
        if frame and frame:IsShown() and member.active == "upcoming" then selectPage("upcoming") end
    end)
    GuildHealthTab:RegisterMessage("GuildParagon_JoinDateChanged", function()
        if frame and frame:IsShown() and member.active == "upcoming" then selectPage("upcoming") end
    end)
    GuildHealthTab:RegisterMessage("GuildParagon_AltsChanged", function()
        if frame and frame:IsShown() and member.active == "upcoming" then selectPage("upcoming") end
    end)
    GuildHealthTab:RegisterMessage("GuildParagon_NicknamesChanged", function()
        if frame and frame:IsShown() and member.active == "upcoming" then selectPage("upcoming") end
    end)
    GuildHealthTab:RegisterMessage("GuildParagon_UpcomingDatesSettingsChanged", function()
        if frame and frame:IsShown() and member.active == "upcoming" then selectPage("upcoming") end
    end)
    GuildHealthTab:RegisterMessage("GuildParagon_RosterScanned", GP:MonitorWrap("listener.healthAccess", function()
        if frame and frame:IsShown() then refreshAccess() end
    end))
    GuildHealthTab:RegisterMessage("GuildParagon_RecruitmentSettingsChanged", function()
        if frame and frame:IsShown() then refreshAccess() end
    end)
    GuildHealthTab:RegisterMessage("GuildParagon_RecruitmentChanged", function()
        if frame and frame:IsShown() and member.active == "recruitment" then
            selectPage("recruitment")
        end
    end)

    frame.OnSelected = refreshAccess
    frame.OnAccessChanged = refreshAccess
    frame.OnDeselected = function()
        if member.active == "housing" and member.housingContent and member.housingContent.OnDeselected then
            member.housingContent:OnDeselected()
        end
    end
    frame.OnAccessLost = function()
        GuildHealthTab:UnregisterAllMessages()
        clearOfficerPageState()
    end
    refreshAccess()
...
function GuildHealthTab:Build(parent)
    local GuildHealth = GP:GetModule("GuildHealth")
    if not GuildHealth:CanUsePage("overview") then
        return buildMemberShell(parent)
    end

    local L = GP.L
    frame = CreateFrame("Frame", nil, parent)
    frame:SetAllPoints()
    frame.accessMode = "officer"
    frame.accessSignature = pageAccessSignature(GuildHealth)
    frame.cards = {}
    ...
    local function addViewButton(key, label)
        if not GuildHealth:CanUsePage(key) then return nil end
        local button = createFilterButton(viewBar, label, function() selectView(key) end)
        button.key = key
        button:SetWidth(key == "info" and 60 or key == "upcoming" and 120 or 100)
    ...
    addViewButton("info", L["Info"])
    addViewButton("attention", L["Attention"])
    addViewButton("recruitment", L["Recruitment"])
    addViewButton("housing", L["Housing"])
    addViewButton("upcoming", L["Upcoming Dates"])
    addViewButton("exports", L["Exports"])

    local guildRecruitment = Theme:CreateButton(viewBar, L["Guild Recruitment"])
    guildRecruitment:SetPoint("LEFT", frame.viewButtons[#frame.viewButtons], "RIGHT", 8, 0)
    guildRecruitment:SetWidth(150)
    frame.guildRecruitment = guildRecruitment
    guildRecruitment:SetShown(GP.UI.GuildRecruitment:IsOfficer())
    GP:RegisterOptionalEvent(guildRecruitment, "GUILD_RANKS_UPDATE")
    guildRecruitment:RegisterEvent("PLAYER_GUILD_UPDATE")
    guildRecruitment:RegisterEvent("GUILD_ROSTER_UPDATE")
    guildRecruitment:SetScript("OnEvent", function(self)
        self:SetShown(GP.UI.GuildRecruitment:IsOfficer())
    end)
    guildRecruitment:SetScript("OnClick", function() GP.UI.GuildRecruitment:Open() end)
    guildRecruitment:SetScript("OnEnter", function()
        GameTooltip:SetOwner(guildRecruitment, "ANCHOR_RIGHT")
        GameTooltip:SetText(L["Open Blizzard's Guild Finder listing editor. Use Accept there to save changes."])
        GameTooltip:Show()
    end)
    guildRecruitment:SetScript("OnLeave", function() GameTooltip:Hide() end)

    frame.viewPages.info = CreateFrame("Frame", nil, viewContent)
    frame.viewPages.info:SetAllPoints()
    GP.UI.GuildInfoPage:Build(frame.viewPages.info)
    ...
    local recruitmentPage
    if GuildHealth:CanUsePage("recruitment") then
        recruitmentPage = CreateFrame("Frame", nil, viewContent)
    ...
    if not GP:IsForeverClient() then
        local housingPage = CreateFrame("Frame", nil, viewContent)
        housingPage:SetAllPoints()
        frame.viewPages.housing = housingPage
        if GP.UI.HousingMap and GP.UI.HousingMap.Build then
            local housingContent = GP.UI.HousingMap:Build(housingPage)
            housingPage.OnSelected = function()
                if housingContent and housingContent.OnSelected then housingContent:OnSelected() end
            end
            housingPage.OnDeselected = function()
                if housingContent and housingContent.OnDeselected then housingContent:OnDeselected() end
            end
        end
    end
    local upcomingPage
    if GuildHealth:CanUsePage("upcoming") then
        upcomingPage = CreateFrame("Frame", nil, viewContent)
        upcomingPage:SetAllPoints()
        frame.viewPages.upcoming = upcomingPage
        if GP.UI.UpcomingDatesPage and GP.UI.UpcomingDatesPage.Build then
            local upcomingContent = GP.UI.UpcomingDatesPage:Build(upcomingPage)
            upcomingPage.Refresh = function()
                if upcomingContent and upcomingContent.Refresh then upcomingContent:Refresh() end
            end
            upcomingPage.OnSelected = upcomingPage.Refresh
        end
    end
    ...
    local mythicPanel = createOverviewPanel(overviewPage, L[GP:IsForeverClient() and "Level Distribution" or "M+ Ratings"])
    ...
    local function refreshOfficerAccess()
        local currentGuildHealth = GP:GetModule("GuildHealth")
        if frame.accessSignature ~= pageAccessSignature(currentGuildHealth) then
            GuildHealthTab:UnregisterAllMessages()
            if GP.UI.MainWindow then GP.UI.MainWindow:RebuildTab("guildhealth") end
            return true
        end
    ...
    local function debouncedRefresh()
        if refreshOfficerAccess() then return end
        if activeView == "upcoming" and frame.viewPages.upcoming and frame.viewPages.upcoming.Refresh then
            frame.viewPages.upcoming:Refresh()
        end
        requestRefresh()
    end
    GuildHealthTab:RegisterMessage("GuildParagon_RosterScanned", GP:MonitorWrap("listener.health", debouncedRefresh))
    ...
    GuildHealthTab:RegisterMessage("GuildParagon_UpcomingDatesSettingsChanged", debouncedRefresh)
    GuildHealthTab:RegisterMessage("GuildParagon_RecruitmentSettingsChanged", debouncedRefresh)
    GuildHealthTab:RegisterMessage("GuildParagon_HousingChanged", function()
        if GP.UI.HousingMap then GP.UI.HousingMap:Refresh() end
    end)
    ...
    frame.OnAccessChanged = refreshOfficerAccess
    frame.OnDeselected = function()
        refreshPending = false
        local activePage = activeView and viewPage(activeView)
        if activePage and activePage.OnDeselected then activePage:OnDeselected() end
    end
    frame.OnAccessLost = function()
        GuildHealthTab:UnregisterAllMessages()
        clearOfficerPageState()
