...
local function countLines(counts)
    ...
    return {
        ...
        string.format("Tools: Rules %d, Ignores %d, Ban %d, Recruit %d, Companion %d, Log %d",
            counts.macroRules or 0, counts.macroIgnores or 0, counts.bans or 0,
            (counts.recruitmentBlacklist or 0) + (counts.recruitmentItems or 0),
            counts.companionSettings or 0, counts.log or 0),
    }
end
...
local function createPeerRow(parent)
    ...
    row.details:SetHeight(22)
    row.details:SetJustifyH("LEFT")
    row.details:SetJustifyV("TOP")
...
local function updatePeerRow(row, peer)
    ...
    local details = detailText(peer.details)
    row.details:SetText(details)
    row.countLine1:ClearAllPoints()
    row.countLine1:SetPoint("TOPLEFT", 4, (details and #details > 44) and -35 or -23)
    row.countLine1:SetPoint("RIGHT", -8, 0)
...
function GuildSyncTab:Build(parent)
    ...
    categoriesText:SetJustifyV("TOP")
    ...
    GuildSyncTab:RegisterMessage("GuildParagon_RosterScanned", GP:MonitorWrap("listener.syncUI", debouncedRefreshStatus))
...
function GuildSyncTab:RefreshStatus(frame)
    ...
    if guildData and guildData.fullLogReplaceCompletedAt then
        bootstrapStatus = string.format(L["replaced %s from %s — imported %d of %d record(s)"],
            GP:DisplayDate("%Y-%m-%d %H:%M", guildData.fullLogReplaceCompletedAt),
    ...
    elseif guildData and guildData.fullLogBootstrapCompleted then
        bootstrapStatus = string.format(L["complete %s from %s — merged %d of %d record(s)"],
            guildData.fullLogBootstrapCompletedAt and GP:DisplayDate("%Y-%m-%d %H:%M", guildData.fullLogBootstrapCompletedAt) or L["never"],
    ...
    local function insertIf(value, line)
        if value then table.insert(categoryLines, line) end
    end

    if GuildSync.lastNicknameProjectionAt or GuildSync.lastNicknameReplayAt
        or (GuildSync.nicknameIgnoredEqualCount or 0) > 0
        or (GuildSync.nicknameIgnoredOlderCount or 0) > 0
        or (GuildSync.nicknameConflictSuppressionCount or 0) > 0 then
        table.insert(categoryLines, string.format("Nickname: %d projected, %d replayed, %d ignored",
            GuildSync.nicknameProjectionCount or 0,
            GuildSync.nicknameReplayCount or 0,
            (GuildSync.nicknameIgnoredEqualCount or 0) + (GuildSync.nicknameIgnoredOlderCount or 0)))
    end
    ...
    if admission then
        local active, deferred = admission:GetStats()
        table.insert(categoryLines, string.format(L["Full/safety receive slots: %d/%d; %d peers deferred"], active, admission:GetReceiveLimit(), deferred))
    end
    insertIf(pendingCount > 0, string.format("Pending transfers: %d (oldest %ds, idle %ds, %d/%d %s chunks)",
        pendingCount, math.floor(oldestAge or 0), math.floor(oldestIdle or 0),
        oldestReceived or 0, oldestTotal or 0, tostring(oldestOp or "?")))
    insertIf((GuildSync.staleRawTransferCount or 0) > 0, string.format("Stale/abandoned: %d total, last %s",
        GuildSync.staleRawTransferCount or 0, formatWhen(GuildSync.lastStaleRawTransferAt)))
    insertIf(GuildSync.lastRawMissingRequestAt, string.format("Raw retry: asked %s (%s)",
        formatWhen(GuildSync.lastRawMissingRequestAt), tostring(GuildSync.lastRawMissingRequestDetail)))
    insertIf((not GuildSync.lastRawMissingRequestAt) and GuildSync.lastRawMissingReplyAt, string.format("Raw retry: sent %s (%s)",
        formatWhen(GuildSync.lastRawMissingReplyAt), tostring(GuildSync.lastRawMissingReplyDetail)))
    insertIf(GuildSync.lastRejectedAt, string.format("Rejected: %s (%s)",
        formatWhen(GuildSync.lastRejectedAt), tostring(GuildSync.lastRejectedReason or "?")))
    insertIf(GuildSync.lastRawChunkSendFailedAt, string.format("Chunk send failed: %s (%s)",
        formatWhen(GuildSync.lastRawChunkSendFailedAt), tostring(GuildSync.lastRawChunkSendFailedDetail)))
    insertIf(GuildSync.lastIgnoredAt and GuildSync.lastIgnoredReason ~= "self echo", string.format("Ignored: %s from %s (%s)",
        formatWhen(GuildSync.lastIgnoredAt), compactName(GuildSync.lastIgnoredFrom), tostring(GuildSync.lastIgnoredReason or "?")))

    -- Show self-check detail only when it looks abnormal; routine local echo
    -- filtering is expected and too noisy for the release diagnostics panel.
    local detail = GuildSync.lastIgnoredDetail
    if detail and detail.method == "guid" and GuildSync.lastIgnoredReason ~= "self echo" then
        table.insert(categoryLines, string.format(
            "Self-check: %s echo filtered", tostring(detail.method or "?")))
    elseif detail and detail.method ~= "guid" then
        table.insert(categoryLines, string.format(
            "Self-check: %s via %s", tostring(detail.method or "?"), compactName(detail.sender)))
    end
