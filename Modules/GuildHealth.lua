...
function GuildHealth:GetCurrentMythicScore(player, seasonStart)
    if GP:IsForeverClient() then return nil end
    if not player then return nil end
    local score = GP:SafeNumber(player.mythicScore, nil)
    if not score or score < 0 or score >= math.huge then return nil end
    local login = {
        online = GP:SafeBool(player.online, false),
        onlineSince = GP:SafeNumber(player.onlineSince, nil),
        lastOnline = GP:SafeNumber(player.lastOnline, nil),
    }
    if not loggedInSince(login, seasonStart or currentMythicSeasonStart()) then return nil end
    return score
end
...
function GuildHealth:BuildLevelDistribution(guildData)
    local result = {total=0, known=0, unknown=0, buckets={}}
    for i=1,6 do
        result.buckets[i] = {label=string.format(GP.L["Level %s-%s"], (i-1)*10+1, i*10), count=0}
    end
    for _, player in pairs(guildData.roster or {}) do
        result.total = result.total + 1
        local level = GP:SafeNumber(player.level, nil)
        if level and level >= 1 and level <= 60 and level == math.floor(level) then
            local bucket = result.buckets[math.ceil(level/10)]
...
local function buildMythicRatings(guildData)
    if GP:IsForeverClient() then return {total=0, rated=0, average=0, buckets={}} end
    local buckets = {
    ...
    for _, player in pairs(guildData.roster or {}) do
        total = total + 1
        local score = GuildHealth:GetCurrentMythicScore(player, seasonStart) or 0
        if score <= 0 then
...
local function buildOverview(guildData, guildKey, context)
    ...
    return {
        ...
        levelDistribution = GP:IsForeverClient() and GuildHealth:BuildLevelDistribution(guildData) or nil,
    }
end
...
local PAGE_ACCESS = {
    info = function() return GP:SafeCall(IsInGuild, false) == true end,
    overview = function()
        return GP:IsOfficer()
    end,
    attention = function()
    ...
    recruitment = function()
        ...
        return Recruitment and Recruitment.CanUse and Recruitment:CanUse() or false
    end,
    housing = function()
        if GP:IsForeverClient() then return false end
        return IsInGuild and IsInGuild() and defaultGuildKey() ~= nil
    end,
    upcoming = function()
        return IsInGuild and IsInGuild() and defaultGuildKey() ~= nil
    end,
...
function GuildHealth:CanUsePage(pageID)
    local predicate = PAGE_ACCESS[pageID]
    return predicate and predicate() and true or false
end

function GuildHealth:CanUse()
    for pageID in pairs(PAGE_ACCESS) do
        if self:CanUsePage(pageID) then return true end
    end
    return false
end

function GuildHealth:GetRecruiterRetentionSummary(guildKey, options)
    if not self:CanUsePage("overview") or not self:CanUsePage("recruitment") then
        return { cohorts = RETENTION_COHORTS, selectedCohort = tonumber(options and options.cohortDays) or 30, recruiters = {}, totalRecruiters = 0, tooNew = 0 }
    end
...
function GuildHealth:GetSummary(guildKey, options)
    if not self:CanUsePage("overview") then return nil end
    guildKey = guildKey or defaultGuildKey()
    local guildData = guildDataFor(guildKey)
