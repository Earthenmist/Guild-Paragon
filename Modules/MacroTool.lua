...
local function maxAccountMacros()
    ...
    return (macroConsts and macroConsts.MAX_ACCOUNT_MACROS) or MAX_ACCOUNT_MACROS or 120
end
...
local function noteBlob(guildKey, guid, player)
    ...
    return table.concat({
        ...
        Nicknames:GetSharedNickname(guildKey, guid) or "",
    }, "\n")
end
...
local function publicNotesEmpty(guildKey, guid, player)
    return strtrim(table.concat({
        ...
        GP:GetModule("Nicknames"):GetSharedNickname(guildKey, guid) or "",
    }, "")) == ""
end
...
local RULE_FIELDS = {
    ...
    "requireTextEmptyOnly", "safeText", "safeTextAllNotes", "requiredLabelId", "requiredLabelGuild",
...
local function copyRule(rule, ts)
    ...
    if rule.action == "label-filter" then out.action = rule.labelAction end
...
local function resetWorkingRule(settings)
    ...
    settings.requiredLabelId = nil
    settings.requiredLabelGuild = nil
...
function MacroTool:GetSettings()
    ...
    if type(s.labelRuleNames) ~= "table" then s.labelRuleNames = {} end
...
function MacroTool:GetExecutionHotKey()
    return GetBindingKey and GetBindingKey(executionBindingAction()) or ""
end

function MacroTool:GetExecutionHotKeyText()
    if not GetBindingKey then return GP.L["None"] end
    local first, second = GetBindingKey(executionBindingAction())
    if first and second then return first .. " / " .. second end
    return first or GP.L["None"]
end

function MacroTool:IsExecutionHotKeyBound()
    if not GetBindingKey then return nil, GP.L["WoW keybinding APIs are not available right now."] end
    local key = self:GetExecutionHotKey()
    return key ~= "", key ~= "" and executionBindingAction() or nil
end

function MacroTool:EnsureExecutionHotKeyBound()
    local bound, detail = self:IsExecutionHotKeyBound()
    if bound == nil then return false, detail end
    if not bound then
        return false, GP.L["Assign the Macro Tool shortcut in Blizzard Keybindings under Guild Paragon."]
    end
    return true, string.format(GP.L["Hotkey %s is bound to %s."], self:GetExecutionHotKeyText(), EXECUTION_MACRO_NAME)
end
...
function MacroTool:SaveRule(name, ts)
    ...
    if settings.savedRules[name] and settings.savedRules[name].requiredLabelId then
        settings.labelRuleNames[name] = true
    end
    if settings.requiredLabelId and settings.savedRules[name] and not settings.savedRules[name].requiredLabelId and not settings.labelRuleNames[name] then
        return false, GP.L["Save label-filtered rules under a new name so older clients cannot use an unrestricted copy."]
    end
    local rule = {}
    for _, key in ipairs(RULE_FIELDS) do
    ...
    ts = ts or time()
    rule.savedAt = ts
    if rule.requiredLabelId then
        rule.labelAction, rule.action = rule.action, "label-filter"
        settings.labelRuleNames[name] = true
    end
    settings.savedRules[name] = rule
    settings.savedRulesUpdated[name] = ts
    settings.currentRuleName = name
    GP:SendMessage("GuildParagon_MacroRuleChanged", name, copyTable(rule), ts, settings.labelRuleNames[name])
    return true
end
...
function MacroTool:LoadRule(name)
    ...
    for _, key in ipairs(RULE_FIELDS) do
        ...
        elseif key == "targetRankIndex" or key == "requiredLabelId" or key == "requiredLabelGuild" then
            settings[key] = nil
        end
    end
...
function MacroTool:DeleteRule(name, ts)
    ...
    ts = ts or time()
    local labelFormat = settings.labelRuleNames[name] or settings.savedRules[name].requiredLabelId ~= nil
    if labelFormat then settings.labelRuleNames[name] = true end
    settings.savedRules[name] = nil
    settings.savedRulesUpdated[name] = ts
    if settings.currentRuleName == name then settings.currentRuleName = nil end
    GP:SendMessage("GuildParagon_MacroRuleChanged", name, nil, ts, labelFormat)
    return true
end
...
function MacroTool:GetSavedRulesForSync()
    local settings = self:GetSettings()
    local rules, timestamps, labelRules, labelTimestamps = {}, {}, {}, {}
    -- Persist format membership across edits and deletion so tombstones use the
    -- same wire namespace and an older client cannot strip the restriction.
    for name, rule in pairs(settings.savedRules) do
        if type(rule) == "table" and rule.requiredLabelId then settings.labelRuleNames[name] = true end
    end
    for name, ts in pairs(settings.savedRulesUpdated) do
        if settings.labelRuleNames[name] then
            labelTimestamps[name] = ts
            if settings.savedRules[name] then labelRules[name] = copyRule(settings.savedRules[name], ts) end
        else
            timestamps[name] = ts
            if settings.savedRules[name] then rules[name] = copyRule(settings.savedRules[name], ts) end
        end
    end
    return rules, timestamps, labelRules, labelTimestamps
end

function MacroTool:SetSavedRuleFromSync(name, rule, ts, labelFormat)
    if not self:CanUse() then return false, GP.L["Macro Tool requires officer access."] end
    name = strtrim(name or "")
    if name == "" or type(ts) ~= "number" then return false end

    local settings = self:GetSettings()
    if not labelFormat and (settings.labelRuleNames[name] or (settings.savedRules[name] and settings.savedRules[name].requiredLabelId)) then return false end
    if labelFormat then
        if rule ~= nil then
            if type(rule) ~= "table" or not ACTIONS[rule.action] then return false end
            if rule.requiredLabelId ~= nil and (type(rule.requiredLabelId) ~= "string" or rule.requiredLabelId == ""
                or rule.requiredLabelGuild ~= GP:GetModule("Roster"):GetGuildKey()) then return false end
        end
        settings.labelRuleNames[name] = true
    elseif type(rule) == "table" and rule.requiredLabelId then
        return false
    end
    settings.savedRulesUpdated[name] = ts
    if type(rule) == "table" then
        ...
        if rule.requiredLabelId then
            settings.savedRules[name].labelAction = settings.savedRules[name].action
            settings.savedRules[name].action = "label-filter"
        end
    else
        settings.savedRules[name] = nil
        if settings.currentRuleName == name then settings.currentRuleName = nil end
    end
    GP:SendMessage("GuildParagon_MacroRuleChanged", name, settings.savedRules[name] and copyTable(settings.savedRules[name]) or nil, ts, labelFormat)
    return true
end
...
-- Resolve once per analysis, using only officer-visible current guild labels.
function MacroTool:GetRequiredLabelMembers(guildKey, settings)
    if not settings.requiredLabelId then return nil end
    local Labels = GP:GetModule("Labels", true)
    if settings.requiredLabelGuild ~= guildKey or not Labels or not Labels:CanUse() then
        return nil, GP.L["Required label is unavailable. Select a current guild label."]
    end
    for _, def in ipairs(Labels:GetAllLabelDefinitions(guildKey, false)) do
        if def.labelId == settings.requiredLabelId then
            local members = {}
            for _, guid in ipairs(Labels:GetPlayersForLabel(guildKey, def.labelId)) do members[guid] = true end
            return members
        end
    end
    return nil, GP.L["Required label is unavailable. Select a current guild label."]
...
function MacroTool:Analyze(guildKey, settingsOverride)
    ...
    local labelMembers, labelError = self:GetRequiredLabelMembers(guildKey, settings)
    ...
    if action == "special" then
        ...
        for altGUID, mainGUID in pairs(alts or {}) do
            ...
            if alt and main then
                ...
                result.requiredLabelId, result.requiredLabelGuild = settings.requiredLabelId, settings.requiredLabelGuild
                if labelMembers and not labelMembers[altGUID] then reject(result, GP.L["Required label is not assigned."]) end
    ...
    for guid, player in pairs(guildData.roster or {}) do
        ...
        result.requiredLabelId, result.requiredLabelGuild = settings.requiredLabelId, settings.requiredLabelGuild
        if labelMembers and not labelMembers[guid] then reject(result, GP.L["Required label is not assigned."]) end
...
function MacroTool:BuildMacroPlan(row, options)
    if type(row) == "table" and row.requiredLabelId then
        local members, err = self:GetRequiredLabelMembers(GP:GetModule("Roster"):GetGuildKey(), row)
        if err or not members or not members[row.guid] then
            return nil, err or GP.L["Required label is no longer assigned. Refresh the preview."]
        end
    end
    options = options or {}
    if not self:CanUse() then return nil, GP.L["Macro Tool requires officer access."] end
...
function MacroTool:BindExecutionMacro()
    -- Blizzard owns these bindings, including intentional unbinding.
    return self:EnsureExecutionHotKeyBound()
end
