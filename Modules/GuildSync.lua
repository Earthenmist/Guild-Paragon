...
function GuildSync:SendFullState(guildKey, requestedBy, helloRequestID, bulkID)
    ...
    local function finishSend()
        ...
        -- 1.1.4 can skip a same-sized nickname snapshot even when it contains
        -- newer timestamps. Replay resolved records through its existing
        -- incremental handler, targeted only to this requester and paced by
        -- the normal ChatThrottleLib route.
        if requestedBy then
            local replay = Nicknames:GetResolvedLegacyReplay(guildKey)
            ...
            for _, record in ipairs(replay) do
                self:SendToPeer({
                    v = PROTOCOL_VERSION, op = "nick", guid = record.guid,
                    nick = record.nick, ts = record.ts,
                }, requestedBy)
            end
        end
