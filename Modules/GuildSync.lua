...
-- Protocol 4 adds explicit alt/main clear tombstones to full-state payloads.
...
local RAW_RECOVERY_REQUEST_GAP_SECONDS = 15
local nextRawRecoveryRequestAt = 0
local lastRawReceiveSweepAt = 0
...
local fullLogClaims = {}
local rawReceives = {}
local completedRawReceives = {}
local outboundRawTransfers = {}
local outboundSequence = 0
local function bulkAdmission()
    local module = GP:GetModule("SyncAdmission", true)
    return module and module.Queue and module or nil
end
...
local function countsFromFullPayload(payload)
    ...
    addCount(counts, "macroRules", (countTable(payload.macroRulesTs or {}) + countTable(payload.labelMacroRulesTs or {})))
    ...
    addCount(counts, "companionSettings", payload.companionSettingsTs and 1 or 0)
    return counts
end
...
local function categoryReceivedCount(category, payload)
    ...
    elseif category == "macroRules" then return (countTable(payload.macroRulesTs or {}) + countTable(payload.labelMacroRulesTs or {}))
...
local function addReceivedDetail(detailed, category, payload)
    ...
    elseif category == "macroRules" then
        detailed.macroRules = detailed.macroRules + (countTable(payload.macroRulesTs or {}) + countTable(payload.labelMacroRulesTs or {}))
...
local function countsForOp(payload)
    ...
    elseif op == "macrorule" or op == "labelmacrorule" then
        counts.macroRules = 1
    elseif op == "recruitmentsettings" then
        counts.recruitmentItems = 1
    elseif op == "safetyfull" and payload.companionSettingsTs then
        counts.companionSettings = 1
...
local function formatReceivedSummary(applied, considered, receivedTotal, skippedCategories, totalCategories)
    applied = tonumber(applied) or 0
    considered = tonumber(considered) or 0
    receivedTotal = tonumber(receivedTotal) or 0
    skippedCategories = tonumber(skippedCategories) or 0
    totalCategories = tonumber(totalCategories) or 0

    local parts = {}
    if receivedTotal > 0 then
        parts[#parts + 1] = string.format("%d received", receivedTotal)
    else
        parts[#parts + 1] = "No records received"
    end

    if applied > 0 then
        parts[#parts + 1] = string.format("%d merged", applied)
        if considered > applied then
            parts[#parts + 1] = string.format("%d newer checked", considered)
        end
    elseif considered > 0 then
        parts[#parts + 1] = string.format("%d newer checked", considered)
        parts[#parts + 1] = "none merged"
    else
        parts[#parts + 1] = "already current"
    end

    if skippedCategories > 0 and totalCategories > 0 then
        parts[#parts + 1] = string.format("%d/%d categories already current", skippedCategories, totalCategories)
    end
...
function GuildSync:IsMutationInProgress(guildKey)
    if applyingRemote then return true end
    for _, exchange in pairs(pendingFullExchanges) do
        if not guildKey or exchange.guildKey == guildKey then return true end
    end
    return false
end
...
local function shortPlayerName(name)
    if GP.IsForeverClient and GP:IsForeverClient() then return tostring(name or "") end
    return tostring(name or ""):match("^([^-]+)") or tostring(name or "")
end

local function findPlayerByGUID(guildKey, guid)
...
local function addRemoteAltMainLog(guildKey, sender, entryType, guid, mainGUID)
    local player = findPlayerByGUID(guildKey, guid)
    if not player then return end
    local extra = { actorName = shortPlayerName(sender), remote = true }
    if mainGUID then
        local mainPlayer = findPlayerByGUID(guildKey, mainGUID)
        extra.mainName = mainPlayer and mainPlayer.name or "?"
    end
    GP:GetModule("EventLog"):Add(guildKey, entryType, guid, player.name, extra)
end

local function applyTimestampMapBatched(label, source, perItem, onDone)
    local keys = {}
    ...
    local function step()
        ...
        GP:MonitorDetail("records", math.max(0, limit-index+1))
        if index <= limit then
    ...
    step = GP:MonitorWrap(label, step)
    step()
end
...
local function localFullPlayerName()
    ...
    if GP.IsForeverClient and GP:IsForeverClient() then return name end
...
local function normalizePeerName(name)
    if not name or name == "" then return nil end
    if GP.IsForeverClient and GP:IsForeverClient() then return tostring(name):lower() end
...
function GuildSync:OnEnable()
    ...
    self:RegisterMessage("GuildParagon_NicknameProjectionCompleted", "OnNicknameProjectionCompleted")
    self:RegisterMessage("GuildParagon_NicknameConflictSuppressed", "OnNicknameConflictSuppressed")
    ...
    self:RegisterMessage("GuildParagon_CompanionSettingsChanged", "OnLocalCompanionSettingsChanged")
...
function GuildSync:MarkPeerUnreachable(target, reason)
    ...
    if bulkAdmission() then bulkAdmission():PeerUnavailable(target) end
...
-- Fresh validated traffic supersedes a prior offline marker, without reviving
-- old transfers whose admission and raw-cache state have already been retired.
function GuildSync:MarkPeerReachable(sender)
    local key = normalizePeerName(sender)
    if not key then return end
    unreachableSyncTargets[key] = nil
    local admission = bulkAdmission()
    if admission and admission.PeerAvailable then admission:PeerAvailable(sender) end
end
...
function GuildSync:IsMe(sender)
    ...
    local full = localFullPlayerName()
...
function GuildSync:PreparePayload(payload)
    ...
    if not noSourceStamp then
        ...
        if GP.IsForeverClient and GP:IsForeverClient() then
            payload.sourceName = localFullPlayerName() or "?"
        else
...
function GuildSync:CompressRawMessage(message)
    if not GP:IsSecretValue(message) and type(message)=="string" then
        GP:MonitorDetail("inputBytes", #message)
    end
    ...
    local ok, compressed = pcall(function()
        -- Prefer short frame work over the smallest possible transfer.
        return LibDeflate:CompressDeflate(message, { level = 1 })
    end)
    ...
    GP:MonitorDetail("outputBytes", #encoded)
    local meta = {
...
function GuildSync:SendRawMessage(message, distribution, target)
    ...
    local immediateFailure = false
    local ok, result = pcall(function()
        local queueName = distribution == "WHISPER" and target and syncQueueName(target) or COMM_PREFIX
        return ChatThrottleLib:SendAddonMessage("NORMAL", COMM_PREFIX, message, distribution, target, queueName, function(_, didSend, sendResult)
            if isSyncWhisper then
                if didSend == false then
                    ...
                    local code = type(sendResult)=="number" and sendResult or -1
                    self.lastRejectedReason = string.format("addon whisper send failed (result %d): %s", code, tostring(target))
                    self.lastRejectedAt = time()
    ...
    return not immediateFailure
end

function GuildSync:SendRawPayload(payload, distribution, target, onDone)
    payload = payload or {}
    if payload.bid and not (bulkAdmission() and bulkAdmission():IsSending(payload.bid)) then return false end
    ...
    GP:MonitorDetail("message", payload.op)
    GP:MonitorDetail("category", payload.category)
    ...
    outboundRawTransfers[transferID] = {
        ...
        target = target,
        bid = payload.bid,
        bulkTarget = payload.bid and bulkAdmission():GetTarget(payload.bid) or nil,
    ...
    local startSent = self:SendRawPayload({
        ...
        helloRequestID = not payload.bid and payload.helloRequestID or nil,
        bid = payload.bid,
    }, distribution, target)
    ...
    local function sendChunk(index)
        if payload.bid and not bulkAdmission():IsSending(payload.bid) then
...
function GuildSync:CleanupRawTransferCaches()
    local now = time()
    for key, expiresAt in pairs(completedRawReceives) do
        if expiresAt <= now then completedRawReceives[key] = nil end
    end
...
function GuildSync:CanRetryBulkSync()
    return not rawRecoveryPaused() and not heavySyncUnsafe()
end

function GuildSync:SendBulkControl(payload)
    payload.v = PROTOCOL_VERSION
    local message = self:Serialize(payload)
    if #message > RAW_SYNC_DIRECT_BYTES then return false end
    return self:SendRawMessage(message, "GUILD")
end

function GuildSync:DropBulkTransfer(id, incomingOnly)
    for key, state in pairs(rawReceives) do
        if state.bid == id then rawReceives[key] = nil end
    end
    for key, exchange in pairs(pendingFullExchanges) do
        if exchange.bid == id then pendingFullExchanges[key] = nil end
    end
    if not incomingOnly then
        for key, state in pairs(outboundRawTransfers) do
            if state.bid == id then outboundRawTransfers[key] = nil end
        end
    end
end
...
function GuildSync:RequestMissingRawChunks(guildKey, state, reason)
    ...
    if rawRecoveryPaused() or heavySyncUnsafe() then return false, "unsafe" end
    if time() < nextRawRecoveryRequestAt then return false, "paced" end
    ...
    nextRawRecoveryRequestAt = time() + RAW_RECOVERY_REQUEST_GAP_SECONDS
...
function GuildSync:ExpireStaleRawReceives()
    local now = time()
    -- Chunk handlers also call this sweep; do not rescan every transfer for
    -- every packet or let that route bypass the recovery safety guards.
    if now == lastRawReceiveSweepAt then return end
    lastRawReceiveSweepAt = now
    for key, state in pairs(rawReceives) do
        ...
        if lastActivityAt and (now - lastActivityAt) > RAW_TRANSFER_STALE_SECONDS then
            ...
            elseif (state.retryAttempts or 0) < RAW_MISSING_RETRY_LIMIT then
                ...
                elseif requestReason == "unsafe" or requestReason == "paced" then
                    -- Deferral must preserve both the chunks and retry budget.
                    self.lastStaleRawTransferAt = now
...
function GuildSync:HandleRawStart(guildKey, payload, sender)
    ...
    if payload.bid and not (bulkAdmission() and bulkAdmission():Accept(sender, payload.bid)) then return end
    local key = rawTransferKey(payload)
    if completedRawReceives[key] and completedRawReceives[key] > time() then return end
...
function GuildSync:HandleRawChunk(guildKey, payload, sender)
    ...
    if not state.chunks[index] then
        ...
        state.lastActivityAt = time()
        state.lastProgressAt = state.lastActivityAt
    end
    ...
    local pieces = state.chunks
    for i = 1, state.total do
        if not state.chunks[i] then return end
    end
    rawReceives[key] = nil
    completedRawReceives[key] = time() + RAW_RESEND_CACHE_SECONDS
    state.chunks = nil
    ...
    runWhenSyncSafe(deferStartedAt, function()
        if state.bid and not bulkAdmission():Accept(sender, state.bid) then return end
        ...
        local function processPayload()
            ...
            self:ProcessPayload(guildKey, innerPayload, state.bid and sender or innerPayload.sourceName or peerName)
        end
        ...
        local function deserializePayload()
            ...
            if not ok or type(innerPayload) ~= "table" or innerPayload.v ~= PROTOCOL_VERSION
                or (state.bid and innerPayload.bid ~= state.bid) then
                completedRawReceives[key] = nil
                if state.bid then bulkAdmission():AbortIncoming(sender, state.bid) end
        ...
        assembled = GP:MonitorCall("sync.concat", table.concat, pieces, "")
        pieces = nil
...
function GuildSync:HandleRawMissing(guildKey, payload, sender)
    self:CleanupRawTransferCaches()
    -- The requester retries later; do not start a resend burst in protected
    -- content or during the post-zone/low-framerate recovery window.
    if rawRecoveryPaused() or heavySyncUnsafe() then return end
    ...
    if transfer.bid and (not bulkAdmission():IsSending(transfer.bid)
        or not peerNamesMatch(requester, transfer.bulkTarget)) then return end
    ...
    local function resendChunk(position)
        if transfer.bid and not bulkAdmission():IsSending(transfer.bid) then return end
...
function GuildSync:ComputeCategoryProgress()
    ...
    self.performancePeerCount = peerCount
    GP:MonitorDetail("peers", peerCount)
    return totals
end
...
function GuildSync:BuildSafetyPayload(guildKey)
    ...
    local Companion = GP:GetModule("Companion", true)
    ...
    local companionSettings, companionSettingsUpdated
    if Companion then
        companionSettings, companionSettingsUpdated = Companion:GetSettingsForSync(guildKey)
    end
    ...
    return {
        ...
        companionSettingsTs = companionSettingsUpdated,
        bans = bans,
        bansTs = bansUpdated,
    }
end
...
function GuildSync:RequestSafetyCatchup(reason)
    ...
    self:Broadcast({ v = PROTOCOL_VERSION, op = "safetyhello", reason = reason or "startup safety", requestID = requestID, bulk = 1 })
...
function GuildSync:SendSafetyState(guildKey, requestedBy, safetyRequestID, bulkID)
    if not guildKey then return end
    local admission = bulkAdmission()
    if not bulkID and requestedBy and admission and admission:Supports(requestedBy) then
        admission:Queue(guildKey, requestedBy, "safety", safetyRequestID)
        return
    end
    if bulkID and not admission:IsSending(bulkID) then return end
...
function GuildSync:BroadcastHello()
    ...
    self:Broadcast({ v = PROTOCOL_VERSION, op = "hello", requestID = requestID, bulk = 1 })
...
function GuildSync:RequestFullLogBootstrap(guildKey, manual)
    ...
    -- localFullPlayerName() nil-guards GetNormalizedRealmName() for the pre-resolve window.
    local requestID = (localFullPlayerName() or "?") .. "-" .. time() .. "-" .. math.random(1000, 9999)
...
function GuildSync:StartFullLogReplaceRequest(guildKey, targetName, targetRole, fallbackTried, candidates, candidateIndex)
    ...
    -- localFullPlayerName() nil-guards GetNormalizedRealmName() for the pre-resolve window.
    local requestID = (localFullPlayerName() or "?") .. "-logreplace-" .. time() .. "-" .. math.random(1000, 9999)
    fullLogRequestsSent[guildKey] = true
    guildData.fullLogBootstrapRequestedAt = time()
    self.fullLogBootstrap = {
        requestID = requestID,
...
function GuildSync:OnLocalAltsChanged(_event, _guildKey, op, guid, mainGUID, ts, source)
    ...
    elseif op == "mainset" then
        self:Broadcast({ v = PROTOCOL_VERSION, op = "main", guid = guid, ts = ts, source = source })
...
function GuildSync:OnNicknameProjectionCompleted(_event, _guildKey, representative, originGUID, revision, records, emitted)
    self.nicknameProjectionCount = (self.nicknameProjectionCount or 0) + 1
    self.lastNicknameProjectionAt = time()
    self.lastNicknameProjectionRepresentative = representative
    self.lastNicknameProjectionOrigin = originGUID
    self.lastNicknameProjectionRevision = revision
    self.lastNicknameProjectionRecords = records or 0
    self.lastNicknameProjectionEmitted = emitted or 0
end

function GuildSync:OnNicknameConflictSuppressed(_event, _guildKey, representative)
    self.nicknameConflictSuppressionCount = (self.nicknameConflictSuppressionCount or 0) + 1
    self.lastNicknameConflictSuppressedAt = time()
    self.lastNicknameConflictRepresentative = representative
end
...
function GuildSync:OnLocalMacroRuleChanged(_event, name, rule, ts, labelFormat)
    ...
    labelFormat = labelFormat or (type(rule) == "table" and rule.requiredLabelId ~= nil)
    if labelFormat and rule then rule = MacroTool:GetSavedRule(name) end
    self:Broadcast({ v = PROTOCOL_VERSION, op = labelFormat and "labelmacrorule" or "macrorule", name = name, rule = rule, ts = ts })
end
...
function GuildSync:OnLocalCompanionSettingsChanged(_event, guildKey)
    if applyingRemote or not guildKey then return end
    self:Broadcast(self:BuildSafetyPayload(guildKey))
end
...
function GuildSync:ProcessPayload(guildKey, payload, sender)
    GP:MonitorDetail("message", payload.op)
    GP:MonitorDetail("peers", self.performancePeerCount)
    local admission = bulkAdmission()
    local peerName = payload.bid and admission and admission:PeerName(sender) or payload.sourceName or sender
    if payload.op == "bulkoffer" or payload.op == "bulkready" or payload.op == "bulkbusy"
        or payload.op == "bulkhold" or payload.op == "bulkdone" or payload.op == "bulkcancel" then
        if admission then admission:OnControl(payload, sender) end
        return
    end
    if admission and payload.bulk == 1 then admission:Observe(sender) end
    if payload.bid then
        local kind = payload.op == "full" and "full" or payload.op == "safetyfull" and "safety" or nil
        if not admission or not admission:Accept(peerName, payload.bid, kind) then return end
    end
    ...
    elseif payload.op == "safetyfull" then
        ...
        C_Timer.After(0, function()
            if payload.bid and not admission:Accept(peerName, payload.bid, "safety") then return end
    ...
    elseif payload.op == "alt" then
        self:ApplyAlt(guildKey, payload, peerName)
    elseif payload.op == "altclear" then
        self:ApplyAltClear(guildKey, payload, peerName)
    elseif payload.op == "main" then
        self:ApplyMain(guildKey, payload, peerName)
    elseif payload.op == "mainclear" then
        self:ApplyMainClear(guildKey, payload, peerName)
    ...
    elseif payload.op == "macrorule" or payload.op == "labelmacrorule" then
...
function GuildSync:OnChatMsgAddon(_event, prefix, message, _distribution, sender)
    ...
    if not GP:IsSecretValue(message) and type(message)=="string" then GP:MonitorDetail("bytes", #message) end
    ...
    if payload.op ~= "bye" then self:MarkPeerReachable(sender) end
    self:ProcessPayload(guildKey, payload, sender)
end

function GuildSync:ApplySafetyState(guildKey, payload, sender)
    if payload.bid and not bulkAdmission():BeginApply(sender, payload.bid, "safety") then return end
    local BanList = GP:GetModule("BanList")
    local Recruitment = GP:GetModule("Recruitment")
    local Companion = GP:GetModule("Companion", true)
    ...
    local function finishSafety()
        if payload.bid and not bulkAdmission():EndApply(sender, payload.bid) then return end
        ...
        self:RecordPeer(sender, "Safety state received", receivedCounts,
            formatReceivedSummary(applied, considered, self.lastSafetySyncCounts.receivedTotal,
                skippedCategories, totalCategories), "snapshot")
        GP:SendMessage("GuildParagon_SyncStatusChanged", guildKey)
        if payload.bid then bulkAdmission():Complete(sender, payload.bid) end
    ...
    finishSafety = GP:MonitorWrap("sync.safetyFinish", finishSafety)

    local function applyCompanionSettings()
        local ts = payload.companionSettingsTs
        local senderIsGM = senderIsGuildMaster(guildKey, sender)
        local current = Companion and Companion:GetSettingsUpdatedAt(guildKey)
        local officerInitialSetup = not senderIsGM
            and GP:IsGuildMaster()
            and (not current or current <= 0)
            and canRequesterViewOfficerData(guildKey, sender)
        if Companion and type(ts) == "number" and (senderIsGM or officerInitialSetup) then
            applyRemote(function()
                if not current or ts > current then
                    considered = considered + 1
                    local ok = Companion:SetSettingsFromSync(guildKey, payload.companionSettings, ts)
                    if ok then
                        applied = applied + 1
                        if officerInitialSetup then
    ...
    applyCompanionSettings = GP:MonitorWrap("sync.safetySettings", applyCompanionSettings)
    ...
    local function applyRecruitmentBlacklist()
        ...
        applyTimestampMapBatched("sync.safetyBlacklist", payload.recruitmentBlacklistTs, function(id, ts)
    ...
    if BanList:CanUse() then
        ...
        applyTimestampMapBatched("sync.safetyBans", payload.bansTs, function(id, ts)
...
function GuildSync:HandleHelloSatisfied(payload)
    ...
    if bulkAdmission() then bulkAdmission():CancelRequest("full", requestID) end
end
...
function GuildSync:HandleSafetySatisfied(payload)
    ...
    if not requestID then return end
    satisfiedSafetyRequests[requestID] = time()
    if bulkAdmission() then bulkAdmission():CancelRequest("safety", requestID) end
end
...
function GuildSync:GetLocalCounts(guildKey)
    ...
    local macroRulesUpdated, macroIgnoresUpdated, labelMacroRulesUpdated = {}, {}, {}
    if MacroTool:CanUse() then
        local _, ruleTs, _, labelTs = MacroTool:GetSavedRulesForSync()
        labelMacroRulesUpdated = labelTs or {}
    ...
    local companionSettingsUpdated
    if Companion then
        local _
        _, companionSettingsUpdated = Companion:GetSettingsForSync(guildKey)
    end
    ...
    local counts = countsFromFullPayload({
        ...
        macroRulesTs = macroRulesUpdated, labelMacroRulesTs = labelMacroRulesUpdated, macroIgnoresTs = macroIgnoresUpdated,
        bansTs = bansUpdated,
        recruitmentSettingsTs = recruitmentSettingsUpdated,
        recruitmentBlacklistTs = recruitmentBlacklistUpdated,
        companionSettingsTs = companionSettingsUpdated,
...
function GuildSync:SendFullState(guildKey, requestedBy, helloRequestID, bulkID)
    local admission = bulkAdmission()
    if not bulkID and requestedBy and admission and admission:Supports(requestedBy) then
        admission:Queue(guildKey, requestedBy, "full", helloRequestID)
        return
    end
    if bulkID and not admission:IsSending(bulkID) then return end
    local Alts = GP:GetModule("Alts")
    ...
    local alts, altsUpdated, mains, mainsUpdated, mainsSource, altsCleared, mainsCleared = {}, {}, {}, {}, {}, {}, {}
    ...
    local labelMacroRules, labelMacroRulesUpdated = {}, {}
    local bans, bansUpdated = {}, {}
    ...
    local function finishSend()
        if helloRequestID and satisfiedHelloRequests[helloRequestID] then
            ...
            if bulkID then admission:AbortSending(bulkID) end
            return
        end
        ...
        if bulkID and #expectedCategories == 0 then admission:AbortSending(bulkID); return end
        local function completeSend()
            ...
            self.lastReplySentCounts = {
                ...
                macroRules = (countTable(macroRulesUpdated) + countTable(labelMacroRulesUpdated or {})), macroIgnores = countTable(macroIgnoresUpdated),
            ...
            addCount(sentCounts, "macroRules", (countTable(macroRulesUpdated) + countTable(labelMacroRulesUpdated or {})))
        ...
        local categoryIndex, replayIndex = 1, 1
        local replay
        local function sendNext()
            if bulkID and not admission:IsSending(bulkID) then return end
            local category = expectedCategories[categoryIndex]
            if category then
                ...
                categoryPayload.bid = bulkID
                if requestedBy then self:SendToPeer(categoryPayload, requestedBy)
                else self:Broadcast(categoryPayload) end
            elseif requestedBy and not replay then
                replay = Nicknames:GetResolvedLegacyReplay(guildKey)
                self.nicknameReplayCount = (self.nicknameReplayCount or 0) + 1
                self.lastNicknameReplayAt = time()
                self.lastNicknameReplayTo = requestedBy
                self.lastNicknameReplayRecords = #replay
            elseif replay and replayIndex <= #replay then
                local limit = math.min(#replay, replayIndex + 19)
                for i = replayIndex, limit do
                    local record = replay[i]
                    self:SendToPeer({v=PROTOCOL_VERSION, op="nick", guid=record.guid,
                        nick=record.nick, ts=record.ts}, requestedBy)
                end
                replayIndex = limit + 1
            else
                completeSend()
        ...
        -- Category compression and legacy replay run in separate frame slices.
        sendNext = GP:MonitorWrap("sync.buildFinish", sendNext)
        C_Timer.After(0, sendNext)
    end

    finishSend = GP:MonitorWrap("sync.buildFinish", finishSend)

    local steps = {
        function()
            alts, altsUpdated, mains, mainsUpdated, mainsSource, altsCleared, mainsCleared = Alts:GetAllForSync(guildKey)
            categoryFields.altsMains = {
                alts = alts, altsTs = altsUpdated,
                mains = mains, mainsTs = mainsUpdated, mainsSource = mainsSource,
                altsCleared = altsCleared, mainsCleared = mainsCleared,
            }
        end,
        function()
        ...
        function()
            if MacroTool:CanUse() then
                macroRules, macroRulesUpdated, labelMacroRules, labelMacroRulesUpdated = MacroTool:GetSavedRulesForSync()
            ...
            categoryFields.macroRules = { macroRules = macroRules, macroRulesTs = macroRulesUpdated,
                labelMacroRules = labelMacroRules, labelMacroRulesTs = labelMacroRulesUpdated }
        end,
        function()
            if BanList:CanUse() then
                bans, bansUpdated = BanList:GetAllForSync(guildKey)
    ...
    local index = 1
    local function measuredStep(step, stepNumber)
        GP:MonitorDetail("step", stepNumber)
        return step()
    end
    local function runNext()
        ...
        index = index + 1
        if step then GP:MonitorCall("sync.buildStep", measuredStep, step, index-1) end
        if index <= #steps then
...
local fullStateCategoryHandlers = {
    bans = function(guildKey, payload, sender, preMergeCounts, ex, done)
        ...
        local logBanEvents = BanList:GetTotalStoredCount(guildKey) > 0
        applyTimestampMapBatched("sync.apply.bans", payload.bansTs, function(id, ts)
            local current = BanList:GetUpdatedAt(guildKey, id)
            if (not current or ts > current) and type(ts) == "number" then
    ...
    recruitmentBlacklist = function(guildKey, payload, sender, preMergeCounts, ex, done)
        local Recruitment = GP:GetModule("Recruitment")
        applyTimestampMapBatched("sync.apply.recruitmentBlacklist", payload.recruitmentBlacklistTs, function(id, ts)
            local current = Recruitment:GetBlacklistUpdatedAt(guildKey, id)
            if (not current or ts > current) and type(ts) == "number" then
                ex.considered = ex.considered + 1
                local ok = Recruitment:SetBlacklistFromSync(guildKey, id, payload.recruitmentBlacklist and payload.recruitmentBlacklist[id], ts)
    ...
    nicknames = function(guildKey, payload, sender, preMergeCounts, ex, done)
        local Nicknames = GP:GetModule("Nicknames")
        ex.nicknameGUIDs = ex.nicknameGUIDs or {}
        applyTimestampMapBatched("sync.apply.nicknames", payload.nicksTs, function(guid, ts)
            ex.nicknameGUIDs[guid] = true
            local current = Nicknames:GetUpdatedAt(guildKey, guid)
            local incoming = (payload.nicks and payload.nicks[guid]) or ""
            local localValue = Nicknames:Get(guildKey, guid)
            if type(ts) == "number" and (not current or ts > current
                or (ts == current and incoming ~= localValue)) then
                ex.considered = ex.considered + 1
                if current and ts == current then
                    if localValue > incoming then incoming = localValue end
                    ts = ts + 1
                end
                local ok = Nicknames:Set(guildKey, guid, incoming, ts)
                if ok then ex.applied = ex.applied + 1 end
            elseif type(ts) == "number" and current and ts < current then
                GuildSync.nicknameIgnoredOlderCount = (GuildSync.nicknameIgnoredOlderCount or 0) + 1
            elseif type(ts) == "number" and current and ts == current and incoming == localValue then
                GuildSync.nicknameIgnoredEqualCount = (GuildSync.nicknameIgnoredEqualCount or 0) + 1
            end
        end, done)
    end,
    ...
    altsMains = function(guildKey, payload, sender, preMergeCounts, ex, done)
        local Alts = GP:GetModule("Alts")
        local senderIsGM = senderIsGuildMaster(guildKey, sender)
        applyTimestampMapBatched("sync.apply.altsMains", payload.altsTs, function(altGUID, ts)
            local current = Alts:GetAltUpdatedAt(guildKey, altGUID)
            if type(ts) == "number" then
                local mainGUID = payload.alts and payload.alts[altGUID]
                local explicitClear = payload.altsCleared and payload.altsCleared[altGUID]
                local localMain = Alts:GetMain(guildKey, altGUID)
                local shouldApply = not current or ts > current
                if senderIsGM and mainGUID and localMain ~= mainGUID then
                    shouldApply = true
                    if current and ts <= current then
                        ts = current + 1
                    end
                end
                if not mainGUID and not explicitClear then
                    shouldApply = false
                end
                if shouldApply then
                    ex.considered = ex.considered + 1
                    local ok = true
                    if mainGUID then
                        ok = Alts:SetMain(guildKey, altGUID, mainGUID, ts, true)
                    else
                        Alts:ClearMain(guildKey, altGUID, ts, true) -- no rejection path
                    end
                    if ok then ex.applied = ex.applied + 1 end
                end
            end
        end, function()
            applyTimestampMapBatched("sync.apply.altsMains", payload.mainsTs, function(guid, ts)
                local current = Alts:GetMainFlagUpdatedAt(guildKey, guid)
                if type(ts) == "number" then
                    local explicitSet = payload.mains and payload.mains[guid]
                    local explicitClear = payload.mainsCleared and payload.mainsCleared[guid]
                    local localSet = Alts:IsMarkedMain(guildKey, guid)
                    local shouldApply = not current or ts > current
                    if senderIsGM and explicitSet and not localSet then
                        shouldApply = true
                        if current and ts <= current then
                            ts = current + 1
                        end
                    end
                    if not explicitSet and not explicitClear then
                        shouldApply = false
                    end
                    if shouldApply then
                        ex.considered = ex.considered + 1
                        if explicitSet then
                            local ok = Alts:SetAsMain(guildKey, guid, ts, payload.mainsSource and payload.mainsSource[guid], true)
                            if ok then ex.applied = ex.applied + 1 end
                        else
                            Alts:UnsetAsMain(guildKey, guid, ts, true) -- no rejection path
    ...
    macroIgnores = function(guildKey, payload, sender, preMergeCounts, ex, done)
        ...
        applyTimestampMapBatched("sync.apply.macroIgnores", payload.macroIgnoresTs, function(guid, ts)
    ...
    macroRules = function(guildKey, payload, sender, preMergeCounts, ex, done)
        local MacroTool = GP:GetModule("MacroTool")
        if not MacroTool:CanUse() then done() return end
        local function applyRules(timestamps, rules, labelFormat, finished)
            applyTimestampMapBatched("sync.apply.macroRules", timestamps, function(name, ts)
                local current = MacroTool:GetSavedRuleUpdatedAt(name)
                if type(ts) == "number" and (not current or ts > current) then
                    ex.considered = ex.considered + 1
                    local ok = MacroTool:SetSavedRuleFromSync(name, rules and rules[name], ts, labelFormat)
                    if ok then ex.applied = ex.applied + 1 end
                end
            end, finished)
        end
        applyRules(payload.macroRulesTs, payload.macroRules, false, function()
            applyRules(payload.labelMacroRulesTs, payload.labelMacroRules, true, done)
        end)
    end,
    ...
    formerMembers = function(guildKey, payload, sender, preMergeCounts, ex, done)
        ...
        applyTimestampMapBatched("sync.apply.formerMembers", payload.formerMembersTs, function(guid, ts)
    ...
    birthdays = function(guildKey, payload, sender, preMergeCounts, ex, done)
        ...
        applyTimestampMapBatched("sync.apply.birthdays", payload.birthdaysTs, function(guid, ts)
    ...
    joinDates = function(guildKey, payload, sender, preMergeCounts, ex, done)
        ...
        applyTimestampMapBatched("sync.apply.joinDates", payload.joinDatesTs, function(guid, ts)
    ...
    notesOfficer = function(guildKey, payload, sender, preMergeCounts, ex, done)
        ...
        applyTimestampMapBatched("sync.apply.notesOfficer", payload.customOfficerNotesTs, function(guid, ts)
    ...
    notesGeneral = function(guildKey, payload, sender, preMergeCounts, ex, done)
        ...
        applyTimestampMapBatched("sync.apply.notesGeneral", payload.customNotesTs, function(guid, ts)
    ...
    labels = function(guildKey, payload, sender, preMergeCounts, ex, done)
        ...
        applyTimestampMapBatched("sync.apply.labels", payload.labelDefinitionsTs, function(labelId, ts)
            ...
            applyTimestampMapBatched("sync.apply.labels", payload.labelAssignmentsTs, function(key, ts)
...
function GuildSync:ReceiveFullStateCategory(guildKey, payload, sender)
    ...
    if payload.bid and not bulkAdmission():BeginApply(sender, payload.bid, "full") then return end
...
function GuildSync:FinishFullExchange(ex)
    ...
    if ex.bid then bulkAdmission():Complete(sender, ex.bid) end
    local BanList = GP:GetModule("BanList")
    local MacroTool = GP:GetModule("MacroTool")
    local CustomNotes = GP:GetModule("CustomNotes")
    local Nicknames = GP:GetModule("Nicknames")

    -- Nickname and alt/main categories are now complete. Reconcile each
    -- affected identity once so no intermediate per-record winner escapes.
    local reconciled = {}
    for guid in pairs(ex.nicknameGUIDs or {}) do
        local representative = Nicknames:GetRepresentativeGUID(guildKey, guid)
        if representative and not reconciled[representative] then
            reconciled[representative] = true
            local result = Nicknames:ReconcileIdentityState(guildKey, representative)
            if result and not result.unresolved and result.needsProjection and result.decision then
                Nicknames:ProjectIdentityDecision(guildKey, result.representative,
    ...
    self:RecordPeer(sender, "Full state received", ex.receivedCounts,
        formatReceivedSummary(self.lastFullSyncCounts.applied, self.lastFullSyncCounts.considered,
            receivedTotal, ex.skippedCategories, #ex.expectedList), "snapshot")
    GP:SendMessage("GuildParagon_SyncStatusChanged", guildKey)
...
function GuildSync:SweepStaleFullExchanges()
    ...
    for key, ex in pairs(pendingFullExchanges) do
        ...
        -- Category assembly can outlast the exchange idle period. Only fresh
        -- chunks from this exchange's admitted transfer extend its lifetime.
        for _, state in pairs(rawReceives) do
            local peerKey = normalizePeerName(ex.sender)
            local samePeer = peerKey and (normalizePeerName(state.sender) == peerKey or normalizePeerName(state.sourceName) == peerKey)
            local sameExchange = ex.bid and state.bid == ex.bid
            if not ex.bid and not state.bid and state.originalOp == "full" and samePeer then
                local candidates = 0
                for _, other in pairs(pendingFullExchanges) do
                    if other.guildKey == ex.guildKey and other.sender == ex.sender then candidates = candidates + 1 end
                end
                sameExchange = candidates == 1
            end
            if state.guildKey == ex.guildKey and samePeer and sameExchange and state.lastProgressAt then
                lastActivityAt = math.max(lastActivityAt or 0, state.lastProgressAt)
            end
        end
...
function GuildSync:ApplyAlt(guildKey, payload, sender)
    ...
    local oldMain = Alts:GetMain(guildKey, guid)
    local applied = false
    applyRemote(function()
        applied = Alts:SetMain(guildKey, guid, payload.main, ts, true)
    end)
    if applied and oldMain ~= payload.main then
        addRemoteAltMainLog(guildKey, sender, "altlinked", guid, payload.main)
    end
end

function GuildSync:ApplyAltClear(guildKey, payload, sender)
    local guid, ts = payload.guid, payload.ts
    ...
    local applied = false
    applyRemote(function()
        applied = Alts:ClearMain(guildKey, guid, ts, true)
    end)
    if applied and mainGUID then
        addRemoteAltMainLog(guildKey, sender, "altcleared", guid, mainGUID)
    end
end

function GuildSync:ApplyMain(guildKey, payload, sender)
    local guid, ts = payload.guid, payload.ts
    if not guid or type(ts) ~= "number" then return end
    local Alts = GP:GetModule("Alts")
    local current = Alts:GetMainFlagUpdatedAt(guildKey, guid)
    if current and ts <= current then return end
    local wasSet = Alts:IsMarkedMain(guildKey, guid)
    local applied = false
    applyRemote(function()
        applied = Alts:SetAsMain(guildKey, guid, ts, payload.source, true)
    end)
    if applied and not wasSet then
        addRemoteAltMainLog(guildKey, sender, "markedmain", guid)
    end
end

function GuildSync:ApplyMainClear(guildKey, payload, sender)
    local guid, ts = payload.guid, payload.ts
    if not guid or type(ts) ~= "number" then return end
    local Alts = GP:GetModule("Alts")
    local current = Alts:GetMainFlagUpdatedAt(guildKey, guid)
    ...
    local applied = false
    applyRemote(function()
        applied = Alts:UnsetAsMain(guildKey, guid, ts, true)
    end)
    if applied and wasSet then
        addRemoteAltMainLog(guildKey, sender, "unmarkedmain", guid)
    end
end
...
function GuildSync:ApplyNick(guildKey, payload)
    ...
    local Nicknames = GP:GetModule("Nicknames")
    local current = Nicknames:GetUpdatedAt(guildKey, guid)
    local incoming = payload.nick or ""
    if current and ts < current then
        self.nicknameIgnoredOlderCount = (self.nicknameIgnoredOlderCount or 0) + 1
        return
    end
    if current and ts == current then
        local localValue = Nicknames:Get(guildKey, guid)
        if incoming == localValue then
            self.nicknameIgnoredEqualCount = (self.nicknameIgnoredEqualCount or 0) + 1
            return
        end
        if localValue > incoming then incoming = localValue end
        ts = ts + 1
    end
    applyRemote(function() Nicknames:Set(guildKey, guid, incoming, ts) end)

    -- Reconcile only after the incoming origin record is fully applied. The
    -- projection itself runs outside applyingRemote so one bounded sibling
    -- wave can reach stable clients; the received origin is not echoed back.
    local result = Nicknames:ReconcileIdentityState(guildKey, guid)
    if result and not result.unresolved and result.needsProjection and result.decision then
        Nicknames:ProjectIdentityDecision(guildKey, result.representative,
            result.decision, true, guid)
    end
end
...
function GuildSync:ApplyMacroRule(payload)
    ...
    local current = MacroTool:GetSavedRuleUpdatedAt(name)
    if current and ts <= current then return end
    applyRemote(function() MacroTool:SetSavedRuleFromSync(name, payload.rule, ts, payload.op == "labelmacrorule") end)
end
