...
function Nicknames:GetRepresentativeGUID(guildKey, guid)
...
function Nicknames:GetIdentityGUIDs(guildKey, guid)
    local data = getGuildData(guildKey)
...
function Nicknames:ReconcileIdentityState(guildKey, guid)
    local data = getGuildData(guildKey)
    if not data then return nil end
...
function Nicknames:ProjectIdentityDecision(guildKey, guid, decision, emitMessages, excludeMessageGUID)
    local data = getGuildData(guildKey)
...
function Nicknames:NormalizeIdentityState(guildKey)
    ...
    for guid in pairs(collectKnownGUIDs(data)) do
        ...
        if representative and not reconciled[representative] then
            ...
            local result = self:ReconcileIdentityState(guildKey, representative)
