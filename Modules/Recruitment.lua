...
function Recruitment:InviteMemberByName(target,guildKey)
    ...
    if GP:IsSecretValue(target) or type(target)~="string" then return false,GP.L["Enter a character name, optionally followed by -Realm."] end
    target=target:match("^%s*(.-)%s*$")
    if target=="" or #target>96 or target:find("[%s|%c]") then
        return false,GP.L["Enter a character name, optionally followed by -Realm."]
    end
