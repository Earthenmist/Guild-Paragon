...
local function fullName(name)
    ...
    if GP:IsForeverClient() then return name end
...
local FOREVER_INVALID_ZONES = {
    ["PvP Area"] = {"Warsong Gulch", "Arathi Basin", "Alterac Valley", "Darkspear Islands"},
    ["Dungeon"] = {
        ...
        "Upper Blackrock Spire", "Blackrock Spire", "Dire Maul", "Scholomance", "Stratholme",
        "Hall of Thanes", "Ruins of Lordaeron", "City of Dalaran", "Blackmaw Hold",
        "The Drowned City", "Krol'dok Stronghold", "Alcaz Prison", "The Shaper's Terrace",
    },
    ["Raid"] = {"Molten Core", "Onyxia's Lair", "Blackwing Lair", "Zul'Gurub",
        "Ruins of Ahn'Qiraj", "Temple of Ahn'Qiraj", "Ahn'Qiraj", "Naxxramas",
        "Hyjal Summit", "Barrow Deeps"},
}
...
local function buildLockedInvalidZones()
    ...
    if GP:IsForeverClient() then
        for reason, names in pairs(FOREVER_INVALID_ZONES) do
            for _, name in ipairs(names) do
                out[normalizeZoneName(name)] = {name=name, reason=GP.L[reason]}
            end
        end
        return out
    end
...
local FOREVER_CLASS_IDS = {1, 2, 3, 4, 5, 7, 8, 9, 11}
local function buildClassList()
    local out = {}
    local forever = GP:IsForeverClient()
    -- Class counts are not the highest class ID on clients with gaps.
    for i = 1, forever and #FOREVER_CLASS_IDS or GetNumClasses() do
        local name, classFile = GetClassInfo(forever and FOREVER_CLASS_IDS[i] or i)
...
local FOREVER_RACE_FACTIONS = {
    human="Alliance", dwarf="Alliance", nightelf="Alliance", gnome="Alliance",
    orc="Horde", undead="Horde", scourge="Horde", tauren="Horde", troll="Horde",
    windshaperskyborne="Horde", highorderskyborne="Alliance",
}
local function foreverRaceFaction(raceInfo)
    local name = GP:SafeOptionalString(raceInfo and raceInfo.raceName) or ""
    local token = GP:SafeOptionalString(raceInfo and raceInfo.clientFileString) or ""
    return FOREVER_RACE_FACTIONS[name:lower():gsub("[%s%-]", "")]
        or FOREVER_RACE_FACTIONS[token:lower():gsub("[%s%-]", "")]
end
...
local function buildRaceList()
    ...
    for raceID = 1, MAX_RACE_ID do
        ...
        local faction
        if GP:IsForeverClient() then
            faction = foreverRaceFaction(raceInfo)
        else
            faction = normalized and RACE_NAME_FACTIONS[normalized]
        end
...
local function classFileForName(className)
    if not className then return nil end
    for _, class in ipairs(getClassList()) do
        if class.name == className then return class.id end
    end
    return nil
end
...
local function makeScannerQuery(minLevel, maxLevel, className, raceName, refined)
    local query = {
        ...
        className = className,
        classFile = classFileForName(className),
        raceName = raceName,
...
function Recruitment:CanManuallyInvite()
    return GP:SafeCall(IsInGuild,false)==true and canGuildInvite()
end

function Recruitment:InviteMemberByName(target,guildKey)
    if not self:CanManuallyInvite() then return false,GP.L["Guild invite permission is required."] end
    local safety=GP:GetModule("GuildApplicants",true)
    if not safety or not safety:IsSafe() then return false,GP.L["Guild invites are unavailable during combat, loading or protected instances."] end
    if not guildKey or GP:GetModule("Roster"):GetGuildKey()~=guildKey then
        return false,GP.L["Your guild has changed. Open Guild Invite again."]
    end
    local forever=GP:IsForeverClient()
    local invalid=GP.L[forever and "Enter the character's first name and surname, separated by a space."
        or "Enter a character name, optionally followed by -Realm."]
    if GP:IsSecretValue(target) or type(target)~="string" then return false,invalid end
    target=target:match("^%s*(.-)%s*$")
    if target=="" or #target>96 or target:find("[|%c]") then return false,invalid end
    if forever then
        target=target:gsub(" +", " ")
        if not target:match("^[^%s]+ [^%s]+$") then return false,invalid end
    elseif target:find("%s") then
        return false,invalid
    end
    -- One direct user-confirmed invitation; never enter the recruitment queue.
    return sendGuildInvite(target)
end
...
function Recruitment:IsInvalidZone(zoneName, guildKey)
    ...
    if not GP:IsForeverClient() and normalized == "delves" then return true, GP.L["Delves"] end
...
local function rememberForeverWhoVisibility()
    if not GP:IsForeverClient() then return end
    scannerState.foreverWhoVisibility = {
        whoShown = LFGWhoListFrame and LFGWhoListFrame:IsShown() or false,
        parentShown = LFGParentFrame and LFGParentFrame:IsShown() or false,
    }
end

local function dismissForeverWhoReply()
    local visibility = scannerState.foreverWhoVisibility
    scannerState.foreverWhoVisibility = nil
    if not visibility then return end
    -- Run after native reply handlers, including demand-loaded Who UI.
    C_Timer.After(0, function()
        if InCombatLockdown and InCombatLockdown() then return end
        if not visibility.whoShown and LFGWhoListFrame and LFGWhoListFrame:IsShown() then
            LFGWhoListFrame:Hide()
        end
        if not visibility.parentShown and LFGParentFrame and LFGParentFrame:IsShown() then
            LFGParentFrame:Hide()
        end
    end)
end
...
local function sendWhoQuery(query)
    ...
    rememberForeverWhoVisibility()
    if not GP:IsForeverClient() and FriendsFrame and FriendsFrame.UnregisterEvent then
...
function Recruitment:OnWhoListUpdate()
    ...
    for i = 1, count do
        ...
        -- Class-name searches use partial matching, so a Hunter query can also
        -- return Demon Hunters. Ignore those rows before deduplication so a
        -- later exact Demon Hunter query can still process the same player.
        local matchesQueryClass = candidate
            and (not activeQuery or not activeQuery.classFile or candidate.classFile == activeQuery.classFile)
        if matchesQueryClass then
