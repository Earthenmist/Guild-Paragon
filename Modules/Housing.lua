...
function Housing:OnEnable()
    if not GP:HasHousingService() then return end
    GP:RegisterOptionalEvent(self, "NEIGHBORHOOD_LIST_UPDATED")
    GP:RegisterOptionalEvent(self, "HOUSE_FINDER_NEIGHBORHOOD_DATA_RECIEVED")
    GP:RegisterOptionalEvent(self, "VIEW_HOUSES_LIST_RECIEVED")
end
...
function Housing:StartScan(silent)
    if not GP:HasHousingService() then return false, GP.L["Housing scan API is not available."] end
...
function Housing:MaybeAutoScan(requestedGuildKey)
    if not GP:HasHousingService() then return false end
...
function Housing:GetPreparedVisit(guildKey, guid)
    if not GP:HasHousingService() then return nil end
...
function Housing:PrepareVisit(guildKey, guid)
    if not GP:HasHousingService() then return false, GP.L["Housing visit lookup is unavailable."] end
