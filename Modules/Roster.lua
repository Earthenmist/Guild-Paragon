...
local function hasInaccessibleSecretValue(...)
    if not GP.CanAccessValue then return false end

    for i = 1, select("#", ...) do
        local value = select(i, ...)
        if value ~= nil and not GP:CanAccessValue(value) then
            return true
        end
    end
...
local function fetchLiveMythicScores()
    ...
    if GP:IsForeverClient() then return scores end
    local blocked = scanBlocked()
    if blocked then return scores end
    ...
    if hasInaccessibleSecretValue(clubId) then return scores end
    if not clubId then return scores end

    local ok = pcall(function()
        local members = C_Club.GetClubMembers(clubId)
        if hasInaccessibleSecretValue(members) then return end
        if type(members) ~= "table" then return end

        for _, memberId in ipairs(members) do
            if hasInaccessibleSecretValue(memberId) then return end
            local info = C_Club.GetMemberInfo(clubId, memberId)
            if hasInaccessibleSecretValue(info) then return end
...
-- Uptime-based evidence is valid only in this loaded session, never SavedVariables.
local chatOnlineProofs = {}
local function preserveChatOnlineSignal(guid, online, scanStartedAt)
    local proof = chatOnlineProofs[guid]
    if proof and proof >= scanStartedAt then return true end
    chatOnlineProofs[guid] = nil
    return online
end
...
local function applyChatOnlineSignal(guildKey, guid, player)
    ...
    player.chatOnlineProofAt = nil
    chatOnlineProofs[guid] = GetTime()
end
...
function Roster:OnGuildEventLogUpdate()
    ...
    GP:GetModule("EventAttribution"):Refresh(self.currentGuildKey or self:GetGuildKey())
end
...
local function mergeGuildData(target, source)
    ...
    target.mainsSource = target.mainsSource or {}
    ...
    for guid, sourceFlag in pairs(source.mainsSource or {}) do
        if target.mains[guid] and target.mainsUpdated[guid] == (source.mainsUpdated or {})[guid] then
            target.mainsSource[guid] = sourceFlag
        end
    end
...
function Roster:IsScanInProgress()
...
function Roster:Scan(automatic, onComplete)
    ...
    abort = function(reason, protectedBlocked)
        endShowOfflineBracket()
        scanInProgress = false
        if onComplete then onComplete(false, reason) end
        if rescanRequestedDuringScan and not protectedBlocked then
            rescanRequestedDuringScan = false
            self:RequestScan(0.5)
        elseif protectedBlocked then
            rescanRequestedDuringScan = false
        end
    end
    ...
    gatherBatch = function(startIndex)
        ...
        if blockedNow then
            abort(automatic and nil or blockedReason, true)
            return
        end

        local endIndex = math.min(numMembers, startIndex + ROSTER_SCAN_BATCH_SIZE - 1)
        for i = startIndex, endIndex do
            local ok, name, rankName, rankIndex, level, _, zone, note, officerNote, online, status, classFile, achievementPoints, _, isMobile, _, _, guid = pcall(GetGuildRosterInfo, i)
            if not ok then
                name = nil
            elseif hasInaccessibleSecretValue(name, rankName, rankIndex, level, zone, note, officerNote, online, status, classFile, isMobile, guid) then
                abort(automatic and nil or GP.L["Roster scan paused while combat or protected content is active."], true)
                return
            end
            ...
            if GetGuildRosterLastOnline then
                ...
                elseif hasInaccessibleSecretValue(yearsOffline, monthsOffline, daysOffline, hoursOffline) then
                    abort(automatic and nil or GP.L["Roster scan paused while combat or protected content is active."], true)
                    return
                end
            end
            ...
            achievementPoints = GP:SafeNumber(achievementPoints, nil)
            ...
            if guid and name and name ~= "" then
                ...
                table.insert(scanRows, {
                    ...
                    mythicScore = mythicScore, achievementPoints = achievementPoints,
        ...
        for rowIndex = startIndex, endIndex do
            ...
            if not player and guildData.formerMembers[guid] then
                ...
                if not isInitialRosterPopulation and not Alts:GetMain(guildKey, guid) and not Alts:IsMain(guildKey, guid) then
                    Alts:SetAsMain(guildKey, guid, now, "auto")
                end
            ...
            online = preserveChatOnlineSignal(guid, online, scanStartedAt)
            player.chatOnlineProofAt = nil
            ...
            if row.achievementPoints ~= nil then player.achievementPoints = row.achievementPoints end
    ...
    finishScan = function()
        GP:MonitorDetail("records", numMembers)
    ...
    gatherBatch = GP:MonitorWrap("roster.gather", gatherBatch)
    applyBatch = GP:MonitorWrap("roster.apply", applyBatch)
    departBatch = GP:MonitorWrap("roster.depart", departBatch)
    finishScan = GP:MonitorWrap("roster.finish", finishScan)
    beginApplyPhase = GP:MonitorWrap("roster.beginApply", beginApplyPhase)
    beginDeparturePhase = GP:MonitorWrap("roster.beginDepart", beginDeparturePhase)
    gatherBatch(1)
    return true
end
...
function Roster:GetFactionForGUID(guid)
    guid = GP:SafeOptionalString(guid)
    if not guid or not PlayerLocation or not PlayerLocation.CreateFromGUID
        or not C_PlayerInfo or not C_PlayerInfo.GetRace
        or not C_CreatureInfo or not C_CreatureInfo.GetFactionInfo then
        return nil
    end
    local cached = factionByGUID[guid]
    if cached then return cached.faction, cached.raceID end

    local locationOK, location = pcall(function()
        return PlayerLocation:CreateFromGUID(guid)
    end)
    if not locationOK or not location then return nil end

    local raceOK, raceID = pcall(C_PlayerInfo.GetRace, location)
    raceID = raceOK and GP:SafeNumber(raceID, nil) or nil
    if not raceID then return nil end

    local factionOK, factionInfo = pcall(C_CreatureInfo.GetFactionInfo, raceID)
    local faction = factionOK and type(factionInfo) == "table"
        and GP:SafeOptionalString(factionInfo.groupTag) or nil
    if faction ~= "Alliance" and faction ~= "Horde" then return nil, raceID end
    factionByGUID[guid] = { faction = faction, raceID = raceID }
