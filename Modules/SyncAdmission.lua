...
local LIMIT, BACKOFF, LEASE, MAX_SESSION = 7, 15, 180, 1200
local COMPLETED_TTL = 600
...
local function fullPeerName(peer)
    ...
    if GP.IsForeverClient and GP:IsForeverClient() then return peer end
...
function Admission:GetReceiveLimit() return LIMIT end
...
function Admission:Retire(entry, retry, requestedWait, reason)
    ...
    if retry then
        ...
        local delay = BACKOFF
        -- Older receivers can still require their original ten-minute cooldown.
        if type(requestedWait) == "number" and requestedWait == requestedWait then
            delay = math.max(BACKOFF, math.min(600, math.ceil(requestedWait)))
        end
        delay = delay + math.random(0, 5)
        entry.retryAt = time() + delay
        local detail = reason == "busy" and GP.L["Peer busy; full/safety sync will retry in about %d seconds."]
            or GP.L["Transfer interrupted; full/safety sync will retry in about %d seconds."]
        self:Status(job.peer, "Sync deferred", string.format(detail, delay))
    end
end
...
function Admission:Complete(peer, id)
    ...
    self.incoming[key] = nil
    self.finished[key .. ":" .. id] = time() + COMPLETED_TTL
...
function Admission:OnControl(payload, peer)
    ...
    if op == "bulkoffer" then
        ...
        if slot or count >= LIMIT or (self.denied[key] or 0) > now then
            ...
            self:Status(peer, "Sync deferred", string.format(GP.L["Receive slot busy; peer can retry in about %d seconds."], math.max(0, self.denied[key] - now)))
            return
        end
    ...
    elseif op == "bulkcancel" and slot and slot.id == id then
        ...
        elseif op == "bulkdone" then
            self:Retire(entry, false)
            self:Pump(entry)
        elseif op == "bulkbusy" then
            self:Retire(entry, true, payload.wait, "busy")
        elseif op == "bulkcancel" then
            self:Retire(entry, true)
        end
    end
end
