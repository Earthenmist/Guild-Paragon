...
local function enrichNativeActor(existing,candidate)
    if candidate.actorSource~="guildevent" or existing.guid~=candidate.guid then return false end
    if existing.type=="join" and candidate.type=="join" then
        if enrichJoinAttribution(existing,candidate) then existing.actorSource="guildevent";return true end
    elseif existing.type==candidate.type or (existing.type=="leave" and candidate.type=="removed") then
        if existing.type~="leave" and existing.type~="removed" and existing.type~="promote" and existing.type~="demote" then return false end
        if (existing.type=="promote" or existing.type=="demote")
            and (existing.fromRank~=candidate.fromRank or existing.toRank~=candidate.toRank) then return false end
        if not existing.actor and not existing.removedBy and type(candidate.actor)=="string" and candidate.actor~="" then
            existing.actor=candidate.actor;existing.actorSource="guildevent";existing.type=candidate.type
            return true
        end
    end
    return false
...
function EventLog:IsDuplicate(guildKey, candidate)
    ...
    for i = #log, 1, -1 do
        ...
        if entry.type == candidate.type and entry.guid == candidate.guid
            and entry.ts and math.abs(entry.ts - candidate.ts) <= DEDUPE_WINDOW_SECONDS
            and ((entry.type~="promote" and entry.type~="demote")
                or (entry.fromRank==candidate.fromRank and entry.toRank==candidate.toRank)) then
...
function EventLog:MergeEntries(guildKey, entries)
    ...
    local knownIDs, knownGRMImports, actorEntries = {}, {}, {}
    for _, entry in ipairs(guildData.log or {}) do
        if entry.guid then
            actorEntries[entry.guid]=actorEntries[entry.guid] or {}
            table.insert(actorEntries[entry.guid],entry)
        end
        if entry.id then
            knownIDs[entry.id] = entry
        end
    ...
    for _, candidate in ipairs(entries) do
        ...
        if type(candidate)=="table" and candidate.actorSource=="guildevent" and not isRemoved(guildData,candidate) then
            local existing=candidate.id and knownIDs[candidate.id]
            if not existing and type(candidate.ts)=="number" then
                local count=0
                for _,entry in ipairs(actorEntries[candidate.guid] or {}) do
                    if entry.guid==candidate.guid and type(entry.ts)=="number"
                        and math.abs(entry.ts-candidate.ts)<=DEDUPE_WINDOW_SECONDS
                        and (entry.type==candidate.type or (entry.type=="leave" and candidate.type=="removed"))
                        and ((candidate.type~="promote" and candidate.type~="demote")
                            or (entry.fromRank==candidate.fromRank and entry.toRank==candidate.toRank)) then
                        count=count+1;existing=entry
                    end
                end
                if count~=1 then existing=nil end
            end
            if existing then changedExisting=enrichNativeActor(existing,candidate) or changedExisting end
        end
        if type(candidate) == "table" and candidate.type == "removed" and candidate.guid and type(candidate.ts) == "number"
            and not (candidate.id and knownIDs[candidate.id]) then
            for index = #(guildData.log or {}), 1, -1 do
                local existing = guildData.log[index]
        ...
        if type(candidate) == "table" and type(candidate.ts) == "number" and candidate.type and (candidate.guid or candidate.type == "grmimport")
            ...
            if candidate.id then knownIDs[candidate.id] = candidate end
...
function EventLog:Render(entry, guildKey, colored, linked)
    ...
    elseif entry.type == "promote" then
        if entry.actor then return string.format(L["%s was promoted from %s to %s by %s."], name, entry.fromRank or "?", entry.toRank or "?", entry.actor) end
        return string.format(L["%s was promoted from %s to %s."], name, entry.fromRank or "?", entry.toRank or "?")
    elseif entry.type == "demote" then
        if entry.actor then return string.format(L["%s was demoted from %s to %s by %s."], name, entry.fromRank or "?", entry.toRank or "?", entry.actor) end
    ...
    elseif entry.type == "altlinked" then
        ...
        if entry.actorName and entry.actorName ~= "" then
            return string.format(L["%s tagged %s as an alt of %s."], entry.actorName, name, mainName)
        end
        return string.format(L["%s was tagged as an alt of %s."], name, mainName)
    elseif entry.type == "altcleared" then
        local mainName = displayName(guildKey, nil, entry.mainName, colored, false)
        if entry.actorName and entry.actorName ~= "" then
            return string.format(L["%s unlinked %s from %s."], entry.actorName, name, mainName)
        end
        return string.format(L["%s is no longer linked to %s."], name, mainName)
    elseif entry.type == "markedmain" then
        if entry.actorName and entry.actorName ~= "" then
            return string.format(L["%s marked %s as a main character."], entry.actorName, name)
        end
        return string.format(L["%s was marked as a main character."], name)
    elseif entry.type == "unmarkedmain" then
        if entry.actorName and entry.actorName ~= "" then
            return string.format(L["%s unmarked %s as a main character."], entry.actorName, name)
        end
