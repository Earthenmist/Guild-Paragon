...
local PREVIOUS_IDS = #ids
for _, label in ipairs({
    "listener.nicknames", "nicknames.migrate", "nicknames.normalize",
    "listener.api", "listener.backupModule",
    "listener.companion", "listener.alts", "listener.backup", "listener.bans",
    "listener.macros", "listener.help", "listener.syncUI", "listener.healthAccess",
    "listener.health", "listener.exports", "listener.rosterUI", "listener.settings",
}) do ids[#ids+1] = label end
...
local function newSession()
    ...
    local s = {diagnostics=3, started=time(), ended=time(), clockStart=now(), build=type(build)=="string" and build or "unknown", version=type(version)=="string" and version or "unknown",
...
local categories = {}
for category in string.gmatch("altsMains nicknames notesGeneral notesOfficer birthdays joinDates formerMembers macroRules macroIgnores bans recruitmentItems recruitmentBlacklist labels log", "%S+") do categories[category]=true end
local detailKeys = {"message","category","bytes","inputBytes","outputBytes","records","peers","step","requests","trigger","aliases","identities","known"}
function GP:MonitorDetail(key, value)
    ...
    elseif key=="category" then
        if type(value)~="string" or not categories[value] then return end
    elseif key=="inputBytes" or key=="outputBytes" or key=="aliases" or key=="identities" or key=="known" or key=="bytes" or key=="records" or key=="peers" or key=="step" or key=="requests" or key=="trigger" then
        value=number(value)
...
function Recorder:OnEnable()
    ...
    for _, old in ipairs(type(db.sessions)=="table" and db.sessions or {}) do
        if type(old)=="table" and type(old.operations)=="table" and (#old.operations==TARGETED_IDS or #old.operations==PREVIOUS_IDS) then
            for i=#old.operations+1,#ids do old.operations[i]=stat() end
        end
    end
...
function Recorder:Report()
    ...
    for n,s in ipairs(db.sessions) do
        ...
        lines[#lines+1]="diagnostics="..(s.diagnostics==3 and "3" or s.diagnostics==2 and "2" or "1")
        ...
        local function callLine(r,kind)
            ...
            for _,key in ipairs(detailKeys) do
                local value=r[key]
                if plain(value) and ((key=="message" and type(value)=="string" and messageTypes[value]) or (key=="category" and type(value)=="string" and categories[value]) or (key~="message" and key~="category" and number(value))) then
