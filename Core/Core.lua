...
GP.defaults = {
    profile = {
        ...
        upcomingDates = {
            ...
            includeEstimatedAnniversaries = false,
        ...
        roster = {
            display = {
                ...
                showHousingIcon = false,
                noteColumn1 = "note",
                noteColumn2 = "officerNote",
            },
            auditFilters = {
                issues = false,
                tag = false,
...
function GP:OnEnable()
    ...
    MainWindow:RegisterTab("guildchat", self.L["Guild Chat"], function(parent) return self.UI.GuildChatTab:Build(parent) end,
        function() return self:HasGuildChatHistory() and self:SafeCall(IsInGuild, false) == true end)
    MainWindow:RegisterTab("professions", self.L["Professions"], function(parent) return self.UI.GuildProfessionsTab:Build(parent) end,
        function() return self:GetModule("GuildProfessions"):CanUse() end)
    ...
    MainWindow:RegisterTab("applications", self.L["Applications"], function(parent) return self.UI.GuildApplicantsTab:Build(parent) end,
        function() return self:HasGuildFinder() end)
...
function GP:SlashCommand(input)
    ...
    elseif cmd == "monitor" then
        self.Recorder:Command(rest)
    elseif cmd == "perf" then
        self:PrintPerformance()
    elseif cmd == "housingscan" then
        local ok, err = self:GetModule("Housing"):StartScan()
        self:Print(ok and self.L["Housing scan started."] or (err or self.L["Housing scan could not start."]))
    elseif cmd == "housingdebug" then
        self:GetModule("Housing"):DumpLastScan()
...
function GP:PrintPerformance()
    if UpdateAddOnMemoryUsage then
        GP:MonitorCall("diagnostic.memoryRefresh", UpdateAddOnMemoryUsage)
    end
